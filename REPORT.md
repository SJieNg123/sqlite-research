...
1. **少即是多**：載前 5 個 interior（−54%）比載全部 92 個（−31%）還好。
   原因**不是** syscall overhead（calibration 量出 layers_92 prefetch tool
   自身只跑 ≈14 µs，可忽略），而是 **N 個 `MADV_WILLNEED` 啟動的 async
   readahead 來不及在 first-q 之前完成**——N 越大、有更多 page 還在 transit。
...
3. **整理 layout 對 A 是大勝（−69%），但對 B 反而變慢**——1c type-aware 把
   interior 全部 collocated 到 file 頭、leaves 推到 file 後段。對 A (Zipfian)：
   hot leaves 只 fault 一次就被 cache，之後 query 從 cached interior 直接到
   cached leaf——加速。對 B (uniform)：每筆 query 都打不同 cold leaf、每筆都
   付 interior (file 頭) → leaf (file 後段) 的較長 disk seek 成本。**我們假設
   1c 對 B 變慢的主因是 interior↔leaf physical seek distance 放大**，但尚未
   直接量過 per-query leaf-fault wallclock vs file offset 來證實；論文用前
   建議補一個小實驗確認，或保留 hedged 措辭。
