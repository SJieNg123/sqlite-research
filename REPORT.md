...
| **C** (tail-region uniform) | 2f_slru | **102 µs (−91%)** | **2e_K10 −54%**（501 µs）| **2e_K10 −75%**（268 µs）|
...
(綠=贏/紅=輸)。**2f 兩模型都遠超紅線**(A +1299% / B +879%)；但 **warm-process 下 targeted prefetch(layers_5/2d/2e_K10)三個 workload 都在紅線下**(A −11~14%、B −29~34%、C 2e_K10 −75%)。
...
2. **targeted prefetch(layers_5/2d/2e_K10)的 e2e 取決於部署模型**:**standalone**(含 ~200 µs 冷 open)
   ...
   (A −11~14%、B −29~34%、C 2e_K10 −75%)。在 warm-process 中 baseline 與 prefetch **雙方都不付 open**,故勝出的純粹原因是 **first-query improvement > delivery cost**;`open_us` 只決定 standalone 與 warm-process 兩模型之間的差距,不決定 warm 模型內 prefetch 對 baseline 的勝負。
...
4. **慢 workload 上「access-pattern + hot leaf」最有效益**：C 上 interior-only(2d/layers_92)
   first-q −38~39%、warm-process e2e −20~32%；加 top-10 hot leaf(2e_K10) 才解鎖 first-q −83%、**warm-process e2e −75%(268 µs)**，全矩陣最佳。
...
2. **headline「targeted prefetch 都贏」的 robust 部分是 interior skeleton + A 的 frequency**：每個 workload 的最佳 access-pattern 策略跨 seed robust——**A 2e_K10 −36%（[−50, −23]）、B 2d −25%（[−32, −16]）、C_hit 2d/learned ~−28%**，CI 不跨 0；**C 2e_K10 −55% 則因 not-found 雙峰、magnitude 不穩**（§6.2.8）。
...
> (2)**access-pattern targeted prefetch 在 warm-process 下三個 workload 的 e2e 都有改善**（§6.2.4）——但**穩健且普適的贏面是 page-type interior skeleton**：A 2e_K10 −36%、B 2d −25%、pure-hit C_hit 2d/learned/2e_K10 ~−28%（皆 robust）;**access-frequency 的 leaf 加分只在有真實熱點時才生效**（A 的 skew、C 的 not-found 集中）;C 2e_K10 跨 seed 為 **−55% 雙峰**（首查是 not-found probe 時 ~−70%、是真 hit 時 ~−31%），非 tight robust——tie-break 已修正（§6.2.8）;
