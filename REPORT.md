...
共同 sum 為 end-to-end cold-start real cost。此 cost-accounting 框架揭露
...
- **(C2) Access-pattern frugality**：基於 `mincore()` snapshot 的
  ...
  **−42~43%**);在慢 workload **C 上 2e_K10 達 first-query −81%、warm-process e2e −73%(291 µs)**——
  是全矩陣最有效益的 e2e。
- **(C3) Preprocessing cost-accounting 框架（兩個部署模型）**：針對 SQLite cold-start read path 提出**首個端到端的 cost-accounting 框架**——將 prefetch
...
額外驗證了壓縮 scenario（§6.2.2）下 first-q 仍保持穩定。換言之，**Crotty+22
...
space*"）。對 mobile / embedded SQLite 部署 scenario 而言，這個 memory 優勢非
...
所有實驗 shared一個固定的 reference database（由 builder script 產生），
...
CPU frequency strategy 需特別說明:量測主機 (Ryzen 9950X, `amd-pstate-epp`) 上，真正釘住frequency的是
...
2. **Workload A 上殘餘的 `fq_async − fq_pread` gap(~165–196 µs)給 50 ms 也補不回來**(補回率 ≈ 0,
   落在雜訊內);且兩臂的 `majflt` 完全相同(layers_5 皆 180)。可見這個 gap **不是 readahead 還沒落地的
   時序假象**,而是「async hint」與「同步 pread」之間一個**真實、持續**的成本差(最可能來自 pread 的
   同步循序讀順帶把 readahead window 養熱、讓 query 後續的 cold-leaf fault 更能 overlap,而對確切 hotset
   頁逐頁發的一次性 `WILLNEED` 不會複製這個效果)。
...
- **async delivery 損失非量測時序假象（§3.5.1）**:harness 在 warmer return 後立刻量 first-query。
  ...
  100% 落地,且 Workload A 殘餘的 `fq_async − fq_pread` gap(~165–196 µs)給 50 ms 也補不回(majflt
  兩臂相同)。故 §5 的 async e2e 數字**不因緊湊時序而偏悲觀**;`fq_pread` 是 async「光等」到不了的理想線。
...
- **Single-machine 結果**：所有實驗在同一台 Ryzen 9950X + NVMe 上跑。SSD
  類型 / 機器架構不同的 reproducibility 未做驗證。
