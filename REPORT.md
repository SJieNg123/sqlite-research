...
*（P0 重跑:clean DB、layout orig、async first-query;A/B 在 N=5 落底、C 需 N=92。churned-DB N-sweep 需 churn-checkpoint 設施,維持 pre-P0。）*
...
*（⚠️ pre-P0 數據:churn-checkpoint 實驗未用 P0 重跑——run_p0.py 不含 churn-checkpoint 設施。）*
...
*（⚠️ pre-P0 數據:cgroup 記憶體受限量測未用 P0 重跑——run_p0.py 尚無 cgroup memory-limit 模式。）*
...
*（⚠️ pre-P0 數據:multiprocess cadence 是獨立 harness,不在 run_p0.py 範圍。）*
...
*（⚠️ pre-P0 數據:Workload Z 的 N-sweep 可用 P0 重跑(workload 在 repo 內、非 new_workloads),但尚未跑;P0 N-sweep 參考見 Figure 4。）*
...
*（⚠️ Figure 11/12 為 pre-P0 dense sweep。P0 的 layers_N plateau(coarse N、clean、orig)見 Figure 4;dense 0..92 × 3 layout(clean+churn)未用 P0 重跑——點數龐大、churn 另需 checkpoint 設施。）*
