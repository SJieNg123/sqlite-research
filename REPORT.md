...
8. **嚴謹度補驗：sparse 6-pt N-sweep 大致正確、但漏了 3 個 sweet spot**：原本 layers_N 只在 6 個 N（1/5/10/20/46/92）取樣，補成 **N=0..92 全 93 個值**（A/B/C × 3 layout × clean + churn 共 **~5,580 個 benchmark**）後驗證——9/12 個 cell 的結論不變，但 **A × 1b vacuum 真正最佳是 N=62（−31%，sparse 漏了 12pp）、B × 1c ta 是 N=26（−36%，漏 12pp）、C × 1b vacuum 是 N=87（−57%，漏 8pp）**。U-shape 也不是 sharp、是雜訊偏多的寬 plateau。

   ![Clean DB 3×3 grid：dense N=0..92 sweep vs sparse 6-pt overlay](figures/out/11_nsweep_full.png)

   *圖 11：A/B/C × {1a, 1b, 1c} 全部 N=0..92 dense 曲線（藍線），疊上 sparse 6-pt（紅點）跟 ground-truth 最低（綠星）。少數 cell 的綠星不在紅點上 → sparse 漏掉的 sweet spot。*

   ![Churn DB：A/B/C × N=0..92 dense vs sparse](figures/out/12_nsweep_full_churn.png)

   *圖 12：churned DB 上同樣的 dense vs sparse 對照（10-checkpoint 平均）。Churn 不改變 plateau 形狀，A 在 N≈19 就壓到 16 µs、C 仍然要 N=92 才壓住。*
