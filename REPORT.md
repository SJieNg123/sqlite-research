...
- **Structure-based**（不看歷史，只看 page 結構）
  ...
  - **2c Layers_N**：按 file offset 升序排前 N 個 interior。**等同於
    ...
    B+tree 樹深無 1-to-1 對應。P0 下 A/orig 上 **N≥5 即進 plateau ~−30% first-q**
...
> **📊 P0 master batch（authoritative）**：本章全部數字為 P0 pipeline 重跑(`run_p0.py`,全 cell `cold_pct`=0),權威表見 [overall_results.md](overall_results.md)、原始檔 [`p0_runs/summary_p0.csv`](p0_runs/summary_p0.csv)。核心:first-query 最低為 **2f_slru(−79~90%)**,但其 preproc 使 `e2e` 出局;**e2e 的勝負取決於 baseline 夠不夠慢**——快 workload(A)上 prefetch e2e 不升反降,慢 workload(C)上 **2e_K10 e2e −56%(462µs)**。

### 5.1 Per-workload best methods (overview)

P0 下(layout orig,async first-query;baseline A 497 / B 725 / C 1058 µs),每個 workload
的「first-query 最低」與「end-to-end 最佳(可部署)」常常**不是同一個策略**:

| workload | first-q 最低 | first-q | e2e | end-to-end 最佳(可部署) | e2e | first-q 改善 |
|---|---|---:|---:|---|---:|---:|
| **A** (Zipfian) | 2f_slru | **107 µs (−79%)** | 7489 µs ⚠️ | **baseline（不 prefetch）** | 497 µs | 任何 prefetch e2e 皆更慢 |
| **B** (uniform) | 2f_slru | **105 µs (−85%)** | 7572 µs ⚠️ | layers_5 / 2d（結構式)| 680–698 µs（**−4~6%**) | layers_5 −47% |
| **C** (churn-heavy) | 2f_slru | **102 µs (−90%)** | 1179 µs | **2e_K10（看歷史)** | **462 µs（−56%)** ✅ | 2e_K10 −85% |

> ⚠️ **重要**:「抄上次 cache」(2f SLRU) first-q 三 workload 都最低(−79~90%),但
> **preprocessing 自己花 ~0.9–7.5 ms**(視 working set 大小),**真實 cold start =
> preprocessing + first-q,因此 e2e 反而比 baseline 慢**(A +1407% / B +944% / C +11%)。
> 詳見 §5.5。
>
> P0 的可部署贏家依 baseline 速度而定:**A(快)上沒有 prefetch 策略的 e2e 贏得了 baseline**
> (warmer 啟動 ~250µs 起就 > 省下的時間);**C(慢)上「看歷史」2e_K10 以 ~300µs preproc
> 換到 e2e −56%**,是真正划算的。

![7 種策略 × 3 種 layout 跨 A/B/C 的 first query latency 比較](figures/out/05_strategy_comparison.png)

*圖 5(P0):每個 workload × layout 下各策略的 async first-query(越短越好)。
...
P0 first-query 改善上限(orig,vs baseline):

| 情境 | 只載 interior(layers/2d) | + 熱 leaves / 全 working set | 為什麼 |
|---|---:|---:|---|
...
| **C**（查檔尾新資料）| −40% | **2e_K10 −85%** / 2f −90% | 每筆 cold leaf,但**「看歷史」加載 top-K 熱 leaf 可突破**到 −85% |
...
不是盲目載前 N 個，而是**先觀察哪些 page 真的被用到**，再只載那些。P0,Workload C、
layout orig(baseline 1058 µs;preproc = warmer wall-clock,含 process 啟動):

| 做法 | First-q | First-q 改善 | Preproc | End-to-end | e2e 改善 |
|---|---:|---:|---:|---:|---:|
...
> e2e 462 µs(−56%)。warmer preproc 三者都 ~300–430µs(以 process 啟動為主、與載幾頁
...
A/B ~105 µs、C ~102 µs(−79~90%),比 baseline 497–1058 µs 短一個數量級。這是 §5.1
...
*圖 14(P0)：**真實的 end-to-end cold start**(stacked:底層 first-q + 黃色斜紋
...
#### 5.5.1 每種策略的 preprocessing 開銷（P0）

P0 的 `preproc_us` = **統一交付引擎 `warmer` 的 wall-clock**（含 process fork/exec
啟動,~250 µs 起跳;這是「以獨立 warmer process 部署」的真實成本)。orig 代表值:

| 策略 | 做什麼 | Preproc(warmer wall-clock) | 跟 first-q 比 |
|---|---|---:|---|
| **2c layers_5** | prefetch 前 5 個 interior | **~258 µs** | ~與 first-q 同量級 ⚠️ |
| **2c layers_92** | prefetch 全部 interior | **~390 µs** | 同量級 ⚠️ |
| **2d access-pattern（只 interior）**| 看歷史只載用過的 interior | **~275–300 µs** | 同量級 ⚠️ |
| **2e_K10（interior + 10 熱 leaf）**| + 最熱 10 leaf | **~290–310 µs** | 同量級 ⚠️ |
| **2e_K500（interior + 500 熱 leaf）**| + 最熱 500 leaf | **~0.7–1.1 ms** | > first-q ⚠️ |
| **2f SLRU（抄上次 cache）**| 載整份 resident working set(~0.4k–4.4k page)| **~0.9–7.5 ms** | **比 first-q 大一個數量級** ⛔ |

> **與舊報告的差異**:舊 calibration 量的是「裸 `madvise` syscall」(~1–2 µs);P0 量的是
> **完整 warmer process 的 wall-clock**(含 ~250 µs 啟動)。若把 prefetch **整合進 app**
> (不另起 process),preproc 會降回 syscall 級(µs)、e2e ≈ first-q;但若以獨立 warmer
> 部署,則小 hotset 也要付 ~250–400 µs 啟動。下面以 P0 量到的(獨立 warmer)為準。

#### 5.5.2 真實 cold start 表現（end-to-end,P0）

加上 preprocessing 後,**e2e 的勝負取決於 baseline 夠不夠慢**。對比快(A)與慢(C):

| 策略 | A/orig e2e (vs base 497) | C/orig e2e (vs base 1058) |
|---|---:|---:|
| Baseline | **497** (—) | **1058** (—) |
...
1. **「抄上次 cache」(2f SLRU) first-q 最低(−79~90%)但 e2e 出局**——其 preprocessing
   達 ~0.9–7.5 ms,使 e2e 比 baseline 慢(A 15×、B 10×、C 1.1×)。first-q 的漂亮數字
   是誤導,真實 cold start 看 e2e。
2. **e2e 是否改善取決於 baseline 速度**:在快 workload(A ~497 µs)上,**連最便宜的
   2c/2d/2e 的 e2e 都 > baseline**(warmer ~250–400 µs 啟動 > first-q 省下的時間);
   在慢 workload(C ~1058 µs)上,**access-pattern 2e_K10 以 ~300 µs preproc 換到
   e2e −56%(462 µs)**,是真正划算的贏家。
3. **若把 prefetch 整合進 app(免去獨立 process 啟動),小 hotset 的 preproc 降回
   syscall 級、e2e ≈ first-q**——此時 2c/2d/2e 在所有 workload 都划算;部署形式
   (整合 vs 獨立 warmer)是決定 e2e 的關鍵旋鈕。

> Cadence(圖 8)是同一條 trade-off 的時間版:背景 warmer 每 cadence 秒重暖一次。
> 實測 **cadence=1s/5s 維持 first-q ~26/29 µs**(warm);**cadence=30s/never 退到
> ~281/305 µs**(≈ 沒 prefetch)。即「**依在意 first-q 還是 overhead,在 frontier 上選一點**」,
> 而非「兩個 metric 各有最佳 cadence」。
...
跨整個 P0 實驗 matrix 看到的結構性 finding（robustness 驗證在 §6.2）：

1. **N（prefetch 幾個 interior）的形狀因 (workload, layout) 而異**:P0 dense N-sweep
   (async)顯示——A/Z 在 **N≥5 即 plateau ~−30%**(orig);**N=1 反而比 baseline 慢
   ~+31%**(warmer/madvise 開銷 > coverage);B 全 plateau **~−47%**(leaf-fault 主導);
   **C 要 N=92 才到 −40%**(熱 interior 在 file 中段、按 offset 取前 N 選錯頁)。
   沒有「N=5 universal sweet spot」這種單一敘事——最佳 N 跟 layout/workload 綁定。
2. **沒有通用 best strategy**:first-query 上 2f 通殺(−79~90%),但看 e2e 時要視
   baseline 速度與部署形式(見 #3、§5.5)。
3. **e2e 的勝負取決於 baseline 速度,不是策略本身**:在快 workload(A ~497 µs)上,
...
4. **慢 workload 上「看歷史 + 熱 leaf」最划算**:C 上 interior-only(2d/layers_92)只
...
完全沒衰退**:P0 量到 C 上 2e_K10_static 跨 checkpoint 維持 ~82–86 µs(vs baseline ~580 µs),
...
*圖 7(P0)：DB 被持續 write 後,static t=0 hot pages 在 A/B/C 三種 workload 上都不衰退
(headline panel = layout orig;CSV 另含 vacuum/ta)。C 上 2e_K10_static 跨 11 個 checkpoint
持平 ~82–86 µs;B 上沒有自然熱葉,access-pattern 與結構式打平,但同樣不 decay。*

#### 6.2.2 RAM pressure（cgroup MemoryMax=20M）

**DB ~102 MB、RAM 用 cgroup `MemoryMax=20M` 砍到 20 MB**（`run_p0.py --mem-limit 20M`,
systemd-run --user --scope),first query 的改善**幾乎不受影響**:

- **First query**：P0 量到「20M / 不限」比值**全部落在 0.95–1.07**(54 個 strategy×
  workload×layout cell),因為 first query 只摸到少數 page、不在 reclaim 路徑上。
- **為什麼壓力這麼小**:resident working set(2f hotset ~4.4k page ≈ **17 MB**)其實
  **略小於** 20M cap,hotset 幾乎塞得下、驅逐有限(這也修正了舊「20M ≪ 16MB」的方向錯誤)。
...
*圖 6(P0)：把可用 RAM 砍到 20 MB（A/B/C × 3 layout × 6 策略）。
每 cell 的「20M / 不限」async first-query 比值**全部落在 0.95–1.07**——memory
...
*圖 8(P0)：背景 warmer 每 cadence 秒重暖、前景每 probe 做全機 drop-caches 後量首查。
**Cadence 是個 trade-off 旋鈕**：cadence=1s/5s 維持 first-q ~26/29 µs、cadence=30s/never
退回 ~281/305 µs(≈ 沒 prefetch)。要 first-q warm 就 cadence ≤ query 間隔,要省成本就加大
cadence(代價是 first-q 退回 cold)。依部署在意哪個目標取捨。*

### 6.3 Practical recommendations

| 情境 | 建議做法 | First-q 改善 | End-to-end（P0,獨立 warmer）|
|---|---|---|---|
| **慢 workload(查檔尾/churn,baseline 高)** | 看歷史:interior + 最熱 ~10 leaf(2e_K10) | **−85%**(C) | **e2e −56%(462µs)** ✅ 真正划算 |
| 平均亂查(uniform) | 結構式 layers_5 / 2d | −47%(B) | e2e ≈ 打平(−4~6%) |
| **快 workload(熱門集中,baseline 已快)** | 結構式 layers_5(first-q) | −30%(A) | 獨立 warmer 下 e2e 不優於 baseline;**要贏 e2e 須把 prefetch 整合進 app**(免 ~250µs 啟動) |
| 想要 batch / avg latency | 抄上次 cache(2f SLRU) | −79~90%(first-q) | **e2e 出局**(preproc ~0.9–7.5ms);只給 batch、不給 cold-start critical path |
| 多 process 共用 DB | shared cache + 背景 warmer,cadence ≤ query 間隔 | 成本固定、效益乘 process 數 | cadence=1s 維持 first-q ~26µs |

> P0 兩條鐵則:(1)**2f SLRU 的 first-q 最低但 e2e 出局**,不適合 cold-start critical path;
> (2)**e2e 要贏,要嘛 baseline 夠慢(C 用 2e_K10)、要嘛把 prefetch 整合進 app 免掉 warmer 啟動成本**。
...
- **「Warm process, cold data」cold-start 模型**（§2.2）：跟「process from
  ...
  first-q 約 ~1–3%。不改變結論(2f 的 ~0.9–7.5 ms preprocessing 仍 dominate e2e)。
- **Preprocessing 計法**:P0 的 `preproc_us` = 獨立 warmer process 的 wall-clock(含
  ~250 µs 啟動);整合進 app 部署可降回 syscall 級(見 §5.5),會改變快 workload 上的
  e2e 結論。`read_ahead_kb` 固定 128(主值);{0,512} 掃描需 root、未跑。
...
更重要的觀察：**「抄上次 cache」(2f SLRU) first-q 看起來最低(−79~90%)是誤導**——
其 preprocessing ~0.9–7.5 ms 比 first-q(~105 µs)大一個數量級,**真實 e2e cold start
反而比 baseline 慢**(A 15×、B 10×、C 1.1×)。2f 的價值在「跑完整段」的 avg latency,
不在第一筆。這個 first-q vs e2e 的 trade-off 在 prefetch 文獻中很少被明說。
