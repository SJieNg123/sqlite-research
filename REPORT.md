...
> `drop-caches` wrapper + residency_checker verify）即為本研究採用的 P0
...
**跟我們的差別**：本 paper Level 1 全部在 application + OS 層；規劃的 Level 2
才下放到 SSD line / namespace 隔離——是 future work。
...
所有 measurement cell 透過自研的 `benchmark_harness`（C 程式）執行，
**嚴格遵守 §2.2 表定義的 P0 cold-start protocol**。每個 cell 的執行
順序如下：
...
| **(iv) Post-cold prefetch**（P0 第③層）| 若策略需要 prefetch，呼叫 `--post-cold-script <strategy>.sh` 執行對應 prefetch tool（`prefetch` / `prefetch_layers` / `prefetch_access` / `prefetch_slru` 之一，arg 詳見 §4.2）；baseline cell 跳過此階段 | §2.2 P0 banner 第③層 |
...
hot-set 演化。每個 strategy 的原理與 post-cold-script 對應見
[overall_strategies.md](overall_strategies.md)。
...
> 的數字一律標記為 pre-P0、由 master rerun 取代。
...
median 統計），作為 sanity-check baseline 與 §5 中跨表的 `prefetch_us` 補齊來源。
...
prefetch cost」的關鍵。詳見 [overall_strategies.md](overall_strategies.md) §三。
...
- **Type-aware Physical Segregation (Level 2)**：把 type-aware layout 從
  ...
  layout。在 FEMU SSD emulator 上做（未來工作）。
...
| 其他 papers / blog posts | §2.3 candidate reading list | survey 進行中 |
