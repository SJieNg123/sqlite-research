...
- **(C3) Preprocessing cost-accounting 框架（兩個部署模型）**：針對 SQLite cold-start read path 提出一套**把 prefetch preprocessing 拆解到 OS-syscall 粒度**的 evaluation methodology——將
  ...
  實驗證實 cache-dump(2f_slru) 雖把 first-query 降到最低(**−79 ~ −91%**),但其 **~0.8–7 ms deliver**
...
|---|---:|---:|---:|---:|---:|---:|
| baseline（不 prefetch）| 523 | 0 | 0 | **523** (—) | **523** (—) | — |
| layers_5 | 382 (−27%) | 227 | 70 | 679 (+30%) | **453 (−14%)** | **−5%** [−16,+4] 6/10 · **tie** |
| 2d | 364 (−30%) | 227 | 88 | 678 (+30%) | **452 (−14%)** | **−25%** [−29,−20] 10/10 · robust |
...
**跨 workload 部署可攜性（第二個 OpenWhisk 角色）。** 上述 3600-invocation campaign 只在
canonical YC 一種 workload 上刻畫**策略空間**。為另外檢驗「同一套代表性策略機制能否**跨不同
workload** 被部署、綁定並通過正確性 gate」，我們再跑了一個**單批 block-union** 的 portability
campaign：**468 筆 formal invocation / 234 個 baseline-target pair**，涵蓋**五個 workload 家族**
（YC `native_ycsb_c_read_zipf`、YCu `native_ycsb_c_read_uniform`、YCh01
`native_ycsb_c_hot_hashed_01`、C `read_tail_mixed_20k`、C_hit `read_tail_hit_20k`），單一 live
matrix fingerprint（`a3274bc9…`）、單一 run-config identity（`64f44c3e…`）、bundle
`a7c9736c…`。**39 個被執行的 target plan** 各自對凍結 keyed contract 做過 page-set + offset
parity 驗證，分為 **24 exact-native-plan / 12 semantic-2e-contract-reconstruction /
...
效果估計**（連同下述覆蓋擴充與收尾 campaign，OpenWhisk 五個 byte-frozen campaign 合計 **5376 筆 formal
invocation** = 3600 + 468 + 852 + 456，皆不合併）。上述同一個 order/state effect 也適用於
...
`analysis/descriptive/portability/`、五-campaign 合成見
[openwhisk_thesis_notes.md](deployment/openwhisk/analysis/thesis/openwhisk_thesis_notes.md)。

**部署效益可攜性（65-cell 描述性一致性）。** 為把「策略在 workstation 有效，是否在模擬
serverless 平台（OpenWhisk）同樣有效」講清楚，我們先追加第四個 campaign（portability_ext，
run-config `bf504a28…`、852 筆 formal invocation / 426 pair）補齊 29 個 cell，再以第五個
campaign（portability_full_closure，run-config `a5be8f15…`、live fingerprint `d35708b7…`、
**456 筆 formal invocation / 228 pair**、6 個 block B12–B17）收尾最後 16 個 cell，使
**workstation orig layout 上全部 65 個 canonical 保留 (策略, workload) cell 都有 OpenWhisk 執行覆蓋**
（BOTH=65、WS_ONLY=0；另有 4 個 YC 專屬的 OpenWhisk-only cell 不計入 workstation 覆蓋）。這是
**cell 覆蓋**（執行 + 正確性 + workload/plan 綁定），**不是**協定、layout 或效能等價，也不是延遲或
排序主張；native/WK1 仍是主要受控效能證據。

比較採**相對**首次查詢降幅 `R =（baseline_fq − strategy_fq）/ baseline_fq`（>0 = 較快 = 有效；只有
...
ρ≈0.67（全）/ 0.75（高信心）、方向一致 42/55、|R 差| 中位數 0.114。3 個例外都已如實標示：C/2d 與
C/layers_92 是低信心 single-instance 的符號翻轉，C_hit/2e_K40 是 position-imbalanced(2/7) 的 cell。
...
[portability_ext_audit.md](deployment/openwhisk/analysis/portability_ext_audit.md) 與 lp 交付順序
另表 `analysis/comparison/lp_delivery_order.csv`。
...
| learned_markov_14（LOSO held-out）| −38.2% | **−29.0%** [−36.1,−19.4] | held-out |
| **2e_K10（修正後）**| **−36.6%** | **−27.2%** [−34.6,−17.7] | **== interior skeleton（不再灌水）** |
...
**結論。** 修正後 `2e_K10` 是乾淨的 arm：pure-hit tail 上 **== interior skeleton（~−27%）**，在 Tail-Mixed 上誠實地暴露 not-found 熱點的雙峰。**page-type（interior skeleton）是普適 robust 贏面；access-frequency 的 *leaf* 選擇只在有真實 access 熱點時才生效**——Scattered-Zipf 的 skew，或 Tail-Mixed 的 not-found 集中。**原生 YCSB 亦印證此界**（native-YCSB-generated suite，見 [overall_results.md](https://github.com/wongzinc/sqlite-research-project-sharing/blob/main/overall_results.md) 與 `results/ycsb_full/REPORT_YCSB_FULL.md`）：其 zipfian 是 **logical skew**，散到 dense-rowid DB 的多個實體葉後**未形成單一超熱葉**，故在所測 native configuration 下 **frequency-selected leaf ≈ random leaf**——**hot-leaf bonus 需 verified physical leaf concentration（真實實體葉集中，如 Tail-Mixed 的 not-found 最右葉），而非僅 logical Zipf skew**；此即原生 YCSB 證實 skeleton-first、而 leaf-frequency 槓桿在此不生效的原因。完整分析見 [results/c_hit/FINDINGS.md](https://github.com/wongzinc/sqlite-research-project-sharing/blob/main/results/c_hit/FINDINGS.md)、`results/tiebreak_fix`、`results/c_hit_v2`。
...
- **「Warm process, cold data」cold-start 模型**（§2.2）：跟「process from
  scratch」差約 1–3 µs,對 baseline ~523–1087 µs 來說 < 1%。本研究的 e2e 即以此模型為主
...
- **Workload coverage**：Scattered-Zipf/Uniform-100K/Tail-Mixed 是合成的三種 access pattern;**workload-instantiation 敏感度已用 10-seed sweep 量化**(§6.2.4,access-pattern targeted prefetch 跨 seed robust、structural layers_5 在 Scattered-Zipf/Uniform-100K 落雜訊內)。但這 10 seed 仍是同三種「分佈家族」的不同抽樣;real world 行為可能更複雜（mixed read/write、time-of-day 變化、跨分佈家族),留待後續驗證。**外部效度補強**:另以 **native-YCSB-generated suite**(原生 YCSB 0.17.0,標準 YCSB-A/B/C/D/E/F + generated 客製 configuration;已凍結入 main,見 `NATIVE_YCSB_MANIFEST.json` 與 `results/ycsb_full/`)獨立重跑,確認 skeleton-first breadth(12 讀取 configuration 中位 −38%、11/12 favorable)。**同批 prior-work 對照,跨三種 access pattern**(`results/native_headtohead{,_YCu,_YCh01}/`,baseline+我方+prior-work 同一 batch,在**三個有 10-seed family 的 native config** 各自 10-seed LOSO:canonical YCSB-C=zipf、YCu=uniform、YCh01=hotspot-1%;唯這三個有 seed family 可做 LOSO,driver `tools/native_headtohead{,_multi}.sh`):我方 interior skeleton `2d` 再 canonical 化 first-q **zipf −36.2%[−46.5,−25.9]、uniform −35.9%[−46.1,−25.7]、hotspot −41.6%[−45.7,−37.4]**;prior-work 之 Chen 式 learned Markov 在**三種 pattern 上**於等頁預算下與簡單頻率選頁皆**統計不可分**(learned_14 vs 2f_top14:zipf −23.4 vs −22.6、uniform −6.7 vs −7.2、hotspot −23.2 vs −22.8,CI 三 config 全重疊),且**皆輸給** `2d`/access hotset `2e_K10`(−36~−42%);libprefetch 之收益為遞送順序效應(`lp_shuf` 比 `lp_sorted` 慢 **15.1–15.2×**,三 config 一致,offset 排序遞送在 NVMe 上勝)。故 native 的外部效度不只「我方 vs baseline」,也是「我方 > 學習型/頻率型 prior art」的同機證據。**關鍵梯度直接坐實下句的邊界**:skew 一消失(uniform),learned 與 frequency **一起塌到 −7%、CI 跨 0**(正是 Leaper 指出的 learned-prefetch 無著力點區),而 workload-independent 的 `2d` 仍守 −36%——**structure > learning,且正好在 learning 失效處最明顯**;唯有真實 physical 熱點(hotspot-1%)時 leaf-frequency/learned 才回到 −23%(CI 排除 0)。原生 YCSB 的 zipfian 為 **logical skew**、散到 dense-rowid DB 未形成單一 physical hot leaf,故 leaf-frequency 槓桿在此不加分,**其效益仰賴 verified physical leaf concentration(§6.2.8)、不能只靠 logical Zipf skew 外推**。
...
最低是載整個 working set 的 **2f_slru(−79 ~ −91%)**，structural **layers_5 / 2e_K10**
...
多半比 baseline 慢**(A 14×、B 10×;只有 C 的小 working set warm-process 才 −12%)。2f 的價值在
...
> **OpenWhisk provenance（reproducibility note）**：§5.6 的 formal 證據為**五個 immutable、archived 的 matrix**——
...
> `5ba26fe9…`、schedule_seed 20260828，補齊 workstation-coverage 至 49 個可比 cell），第五個屬 456-invocation 的
> portability-full-closure campaign（run-config `a5be8f15…`、matrix fingerprint `d35708b7…`、blocks B12–B17，
> archived 於 `deployment/openwhisk/evidence/portability_full_closure/956cd15b5db3/`，補齊最後 16 個 workstation-only
> cell 使全部 65 個 canonical 保留 cell 皆有 OpenWhisk 執行覆蓋——BOTH=65、WS_ONLY=0，另有 4 個 YC 專屬 OpenWhisk-only
> cell；此為 **cell 覆蓋**，不是協定、layout 或效能等價）。五者的 bundle hash 與
> execution identity 記於各自 manifest；正規化、descriptive、thesis 合成產物在
> `deployment/openwhisk/analysis/{normalized,descriptive,thesis}/`（portability、portability_ext 與
> portability_full_closure 另有平行的
> `analysis/{normalized,descriptive}/portability{,_ext,_full_closure}/`、[portability_audit.md](deployment/openwhisk/analysis/portability_audit.md)
...
> **strategy-space（3600）、portability（468）、portability-extension（852）與 portability-full-closure（456）分屬回答不同
> 問題的 campaign、合計 5376 formal invocation = 3600 + 468 + 852 + 456，於本文中不被合併為單一效果估計。** 已知
> secondary、portability、portability_ext 與 portability_full_closure 的
...
> `441609e6…` / `64f44c3e…` / `bf504a28…` / `a5be8f15…`，不影響上述任何 identity 或 §5.6 量值。
