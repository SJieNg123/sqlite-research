...
研究（以下numbers全部來自 P0 master batch,authoritative表見 §5 / [overall_results.md](overall_results.md);全 cell `cold_pct`=0）：experiment顯示既有 cache-dump strategy（2f_slru,load整個 resident working set）雖能把 first-query latency 降到最低（**−76 ~ −89%**,A/orig 529→127 µs、C/orig 1096→123 µs），但其 **~0.8–7 ms 的 deliver overhead** 反讓 end-to-end cold start **慢上一個order of magnitude**——這個 trade-off 在既有 prefetch literature中長期被忽略。相對地,**page-type-aware 的structural / access-pattern prefetch（layers_5 / 2e_K10）** 以**極少 syscall** 取得 first-query **−22 ~ −81%**;關鍵在於 end-to-end **取決於部署模型**:我們把 preprocessing 拆成「冷 open(db)(~200 µs,per-layout 常數)」與「deliver(隨 hotset)」,並分兩個模型呈現——**standalone warmer**(另起 process、付冷 open)與 **warm-process / integrated**(app 已在跑、重用 handle、不付冷 open,即本研究主張的部署)。**在 warm-process 模型下,targeted prefetch 在三個 workload 的 e2e 都改善**(A −7~9%、B −29~34%、慢 workload C 的 2e_K10 達 **e2e −73%（291 µs）**);只有 2f 因 deliver 過重而 e2e 多半不具優勢。三條 robustness 軸——**50k write churn**(static t=0 hotset 不decay)、**cgroup `MemoryMax=20M`** memory壓縮(20M/unlimited ratio近 1.0)、與多-process **cadence re-warm**——下conclusion皆穩定。
...
cold first query baseline 落在 **~529–1096 µs** 區間（A 529 / B 760 / C 1096 µs @orig,
...
- **(C3) Preprocessing cost-accounting framework（兩個部署模型）**：據我們所知，首次將 prefetch
  ...
  此 cost-accounting framework 揭露的 trade-off 在既有 prefetch literature中長期被忽略。
...
到看到結果」的end-to-end時間，必須把 prefetch preprocessing 算進去。warmer 把
...
> **本 metric 是本論文 §5.5 core observation的依據**：當 `deliver_us` 與 `first_query_us`
...
> **P0 master batch（authoritative）**：本章 numbers 為 P0 pipeline rerun(全 cell `cold_pct`=0,async 臂,median;baseline A 529 / B 760 / C 1096 µs @orig)。**e2e 以兩個部署模型並陳**:`e2e_std`(standalone warmer,含冷 open(db))與 **`e2e_warm`(warm-process / integrated,handle 已開、不付冷 open;≈ static `effective_first_query`)**——後者是本研究主張的部署(§3.4)。core:first-query 最低為 **2f_slru(−76~89%)**,但 2f 的 ~0.8–7 ms deliver 使其 e2e 多半不具優勢;**warm-process 下 targeted prefetch(layers_5 / 2d / 2e_K10)在三個 workload 的 e2e 都改善**。
...
「e2e best」常常**不是同一個strategy**,而且 **e2e best 取決於部署模型**:
...
>
> **e2e best 依部署模型而定**:**standalone warmer**(另起 process、冷開 DB,多付 ~200 µs)下,
...
**（P0 rerun數據,async 臂,median;authoritative表見 [overall_results.md「P0 master batch 結果」](overall_results.md)。`open`=冷 open(db)、`deliver`=prefetch syscalls;`e2e_std`=fq+open+deliver、`e2e_warm`=fq+deliver。）**
...
> **P0 關鍵**:在 A 這種 baseline 本來就快(~529 µs)的 cell,**standalone warmer 的冷 open(db)(~193 µs)+ deliver 蓋過 first-query 省下的時間 → `e2e_std` 比 baseline 慢(+27~29%)**;但若 prefetch **integrate 進 app**(handle 已開、不付冷 open,= warm-process),**`e2e_warm` 反而比 baseline 快 7~9%**。差別就是那一次冷 open(db)(§3.4 / §5.5)。2f 因 deliver ~7 ms,兩模型都遠輸。
...
|---|---:|---:|---|
| **A**（熱門集中）| −22~26% | 2e_K500 −60% / 2f −76% | hot leaves 多半已暖,interior-only 即有中段效益 |
...
| **+ 最熱的 10 個 leaf node（2e_K10）**| **211** | **−81%** | 222 | 80 | **512 (−53%)** | **291 (−73%)** |

> P0 關鍵:C 上「只載 interior」(2d/layers_92) first-q −37~38%、`e2e_warm` 已 −20~31%;
> **真正解鎖的是加載 top-10 熱 leaf(2e_K10)**——把 first-q 壓到 211 µs(−81%)、
> **`e2e_warm` 291 µs(−73%)/ `e2e_std` 512 µs(−53%)**,全矩陣最佳 e2e。三者 deliver 都小
> (~70–200 µs)、冷 open ~220 µs,所以 e2e 由 first-q 決定,而 2e_K10 的少量熱 leaf 是最有效益的選擇。

### 5.5 The preprocessing trade-off （本 paper 的core observation）

前面所有 first-q numbers都**只算 SQL 第一筆 query 的時間**——但 prefetch 自己也要時間。
**real cold start = preprocessing + first-q**,而 preprocessing 拆成兩個 term:

- **open(db)**:冷開 DB 的 setup(~193–222 µs,per-layout 常數)。**只有「另起 standalone warmer
  process」才付**;app 已在跑、重用既有 handle(本研究主張的部署)則免。
- **deliver**:迭代 hotset + 逐頁 madvise/pread(隨 hotset 大小:小策略 ~70–200 µs、
  2e_K500 ~0.5–0.85 ms、2f ~0.6–7 ms)。

對應**兩個部署模型的 e2e**:
- **standalone-warmer e2e** = first-q + open + deliver(悲觀;另起 process)。
...
灰色那段就是「integrate 進 app 即可省掉」的冷 open(db)。每根 bar 標的是 **warm-process e2e 改善**
(綠=贏/紅=輸)。**2f 兩模型都遠超紅線**(A +1248% / B +843%);但 **warm-process 下 targeted prefetch
(layers_5/2d/2e_K10)三個 workload 都在紅線下**(A −7~9%、B −29~34%、C 2e_K10 −73%)。*
...
> 模型的**唯一差**:standalone warmer 另起 process、冷開 DB 要付它;app 已在跑、重用既有 handle 則免。
> deliver 隨 hotset 大小線性。(舊 calibration 量的「裸 `madvise` syscall」~1–2 µs ≈ 此處 deliver 的下界。)
...
1. **「重載前次 cache」(2f SLRU) first-q 最低(−76~89%)但 e2e 多半不具優勢**——其 deliver
   (載整份 working set)A/B ~7 ms 使 e2e 比 baseline 慢一個量級(兩模型皆然);只有 **C(deliver
   ~0.76 ms)warm-process e2e −19%** 才小贏。first-q 亮眼卻 misleading,real cold start 要看 e2e。
2. **targeted prefetch(layers_5/2d/2e_K10)的 e2e 取決於部署模型**:**standalone**(含 ~200 µs 冷 open)
   ...
   (A −7~9%、B −29~34%、C 2e_K10 −73%)。**唯一的差就是那一次冷 open(db)**。
3. **本研究主張 warm-process**(app 已在跑、prefetch 重用既有 handle):此時 targeted prefetch
   ...
   部署形式(integrate vs standalone warmer)就是決定 e2e 勝負的關鍵knob。
...
2. **沒有通用 best strategy**:first-query 上 2f 全部最低(−76~89%),但看 e2e 時要視
...
3. **e2e 取決於部署模型 + deliver 大小**:**standalone warmer**(另起 process、付 ~200 µs 冷 open)下,
   ...
   **warm-process / integrated**(app 已在跑、重用 handle、不付冷 open)下,**targeted prefetch
   (layers_5/2d/2e_K10)三個 workload 的 e2e 都改善**(A −7~9%、B −29~34%、C 2e_K10 −73%)。
   差別就是那一次冷 open(db)。type-aware layout 在 P0 下把 A/B 的 baseline **推高**(A +31%、B +4%)、
   C 較快(−21%),不是舊 P1「A −69% 大幅改善」。
4. **慢 workload 上「access-pattern + 熱 leaf」最有效益**:C 上 interior-only(2d/layers_92)
   first-q −37~38%、warm-process e2e −20~31%;加 top-10 熱 leaf(2e_K10)才解鎖 first-q −81%、
   **warm-process e2e −73%(291 µs)**,全矩陣最佳。
5. **2f(整份 working set)e2e 多半不具優勢**:deliver A/B ~7 ms 使 e2e 慢一個量級(兩模型皆然);
...
> (2)**targeted prefetch(layers_5/2d/2e_K10)在 warm-process(integrate 進 app、重用 handle、免冷 open)下
> 三個 workload 的 e2e 都改善**——這是本研究主張的部署。
...
拆成「冷 open(db)(~200 µs,per-layout 常數)」與「deliver(隨 hotset)」,並以兩個模型呈現:
**standalone warmer**(另起 process、付冷 open)與 **warm-process / integrated**(app 已在跑、重用 handle、
不付冷 open,即本研究主張的部署)。**在 warm-process 下,targeted prefetch(layers_5/2d/2e_K10)
在三個 workload 的 e2e 都改善**(A −7~9%、B −29~34%、慢 workload C 的 **2e_K10 e2e −73%(291µs)**);
2f 則因 deliver ~0.8–7 ms 而 e2e 多半不具優勢(只有 C 的小 working set 例外)。這些結果在 50k write
churn、cgroup `MemoryMax=20M` memory pressure、與 cadence re-warm三條 robustness 軸下穩定
(全 P0 cell `cold_pct`=0)。

更重要的observation：**「重載前次 cache」(2f SLRU) first-q 看似最低(−76~89%)是misleading**——
其 deliver ~0.8–7 ms 比 first-q(~125 µs)大一個order of magnitude,**real e2e cold start
多半比 baseline 慢**(A 12×、B 10×;只有 C 的小 working set warm-process 才 −19%)。2f 的價值在
「跑full段」的 avg latency,不在第一筆。**而 first-q vs e2e、以及「standalone 含冷 open vs
warm-process 不含」這兩層 trade-off,在既有 prefetch literature中很少被明說**——這是本研究的 core observation。
