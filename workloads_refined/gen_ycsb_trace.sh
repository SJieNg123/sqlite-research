...
#
# YR ρ-sweep: recordcount 不手填 —— 由 yr_sweep.yaml + tools/calc_n.py 由 W 現算
#   (見 spec §2.2 迴圈 / §4.5.3)。字面 N 不得出現在任何「給人複製」的形式裡。
set -euo pipefail
...
python3 "$HERE/tools/ycsb2trace.py" "$RUN_LOG" "$OPS" --workload "$WL" --out "$JSONL"
...
man={
    ...
    "props": props,
    "out": os.path.relpath(out), "n_ops": val["n_ops"],
    "trace_lines": val.get("trace_lines"), "ycsb_ops": val.get("ycsb_ops"),
    "op_mix": val["op_mix_actual"], "op_mix_ops": val.get("op_mix_ops"),
