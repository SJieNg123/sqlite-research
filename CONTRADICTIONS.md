...
| ~~18~~ | ~~理論~~ | ~~「載太多變慢」歸因~~ | ✅ **已解**：歸因改為「N 越大、async readahead 來不及完成」，**不是** syscall overhead（calibration 量出 layers_92 ≈ 14 µs，syscall 本身可忽略）。改寫位置：REPORT §6.1 / overall_results §3 維 N sweep + 主表 | ✅ |
| ~~19~~ | ~~理論~~ | ~~plateau vs U 型退化~~ | ✅ **已解**：統一為「wide-bottom U 形」——N=1→5 急降、N=5..46 plateau、N>46 略升。改寫位置：overall_strategies §2c 標題段 | ✅ |
...
| ~~21~~ | ~~理論~~ | ~~1c 重排對 A/B 相反因果~~ | ✅ **已解**：統一「interior↔leaf physical seek distance 放大」hypothesis，A hot-leaf-cached 抵消、B uniform 每筆付放大 seek 成本。改寫位置：REPORT §6.1 #3（**hedged 措辭**：尚未直接量過 leaf-fault wallclock vs file offset，論文用前建議補小實驗或保留 hedge） | ✅ |
