...
| 2d Access pattern, interior only（**已完成**，見 [strategies/access/](../strategies/access/)）| 攔截每次 page read，存 access count | 區分 1 次 vs 100 次 |
| 2e Access pattern, interior+leaf（**已完成**，見 [strategies/access/](../strategies/access/)）| 同上但含 leaf | 同上 |
...
`residency.csv` 是 [residency_checker](../../pipeline/engine/residency_checker/) 的標準輸出
...
> - **Workload B** = Uniform random point-read (`workload_b.txt`)
> - **Workload C** = High-key uniform read (`workload_c.txt`)
...
runs/                  — warmup.sh、prefetch wrappers、runmatrix.sh、raw results
  workload_a_zipfian.txt → ../../workloads/workload_a.txt
  workload_b_uniform.txt → ../../workloads/workload_b.txt
  workload_c_highkey.txt → ../../workloads/workload_c.txt
