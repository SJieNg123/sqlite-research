...
| ~~**2d/2e × churned DB**~~ | ~~第七維只測 layers_N × churn；access-pattern prefetch 在 DB 持續變動下的衰退曲線未知~~ → **已完成 (minimal sweep)**：1a × C × 10 checkpoints × 50 k 寫入 ops，**靜態 t=0 hotpages 即可**——acc_2d 仍 -50%，acc_2e_K10 仍 -91%。原因：workload C hot leaves ([590k, 610k]) 與 churn insert target (id 600001+) 大幅重疊 → hot-leaf set workload-stable。詳見 [prefetch_churn/runs_access_churn/README.md](prefetch_churn/runs_access_churn/README.md)。剩餘 open：A (Zipfian) + 隨機刪除 churn 下 hot leaves 會旋轉。 |
