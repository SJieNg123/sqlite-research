...
> [第十八維](#第十八維--churn-擴充abc--churn--2c-layers_n--2d--2e_kab--churn--statictk0-hotpages)（**A/B churn 擴充**：A × delete-churn × 2d/2e_K + A/B × N sweep × churn — static t=0 hotpages 不 decay、N-sweep plateau 形狀跨 workload 一致）。
...
| ~~**多 process 場景下 prefetch worker 排程**~~ | ~~第六章已證明 MAP_SHARED 下一個 process prefetch 全部受惠；但 DB 持續 churn 時，多久跑一次 prefetch worker 才合理？~~ → **已完成 (minimal sweep)**：writer + prefetcher + probe 三線程，掃 cadence ∈ {1 s, 5 s, 30 s, never} × 4 rounds × gap=3 s。**cadence=1 s 把 first_q 295 µs → 19 µs (-94%)**；cadence=5 s 只有 ~50% hit rate (177 µs, -40%)；cadence=30 s 等同無 prefetcher。**經驗法則：cadence ≤ gap_s 才可靠 warm**。詳見 [multiprocess/runs_prefetch_cadence/README.md](multiprocess/runs_prefetch_cadence/README.md) |
| ~~**2d/2e × churned DB**~~ | ~~第七維只測 layers_N × churn；access-pattern prefetch 在 DB 持續變動下的衰退曲線未知~~ → **已完整補上**：(1) **C × insert-churn** ([prefetch_churn/runs_access_churn/README.md](prefetch_churn/runs_access_churn/README.md))：acc_2d -50% / acc_2e_K10 -91%，static t=0 hot 不 decay。(2) **A × delete-churn** ([prefetch_churn/runs_access_churn_a/README.md](prefetch_churn/runs_access_churn_a/README.md))：3 arms × 10 checkpoints；**2e_K10 -92.4% / 2d -91.8% / 2e_K50 -91.5%**，hypothesis（delete from id=1 會擾動 A 的 hot leaves）**被推翻** — static t=0 hot 完全沒 decay（Zipfian 熱 keys 散佈寬，集中 delete 沒打到 hot leaves）。**結論：access-pattern × static hot 在 insert/delete 兩種 churn 下都穩定**。(3) **A/B × layers_N × churn** ([prefetch_churn/runs_nsweep_a/README.md](prefetch_churn/runs_nsweep_a/README.md) + [prefetch_churn/runs_nsweep_b/README.md](prefetch_churn/runs_nsweep_b/README.md))：A layers_5 -90.7% / B layers_5 -45.9%，churn 不改變 N sweep 的 plateau 形狀。 |
...
（C × insert-churn × 2d/2e_K10）之後，留下兩個 gap：
...
本維把這兩個 gap 一次補完：B1 跑 3 個 access-pattern arm × 10 churn checkpoint；
B2 跑 N ∈ {0, 1, 5, 10, 20, 46, 92} × 10 churn checkpoint × {A, B} workload。
...
- **Access-pattern prefetch × static t=0 hot 在 churn 下是 production-ready
  的 baseline 模式**——A × delete / C × insert 兩個正交 churn 類型都驗證過，
  完全不需要週期性 re-warmup。
...
- **缺口剩下**：B × access-pattern × churn 還沒測——但 leaf 沒有自然熱點，
  靜態 hot-K 可能無效（access-count ordering 對 uniform reads 退化成隨機
  選 K 個 page）。屬可選未來工作。
