...
2. **越多 prefetch 不一定越好**：perpage 載 92 個 page 比 layers_5 只載 5 個還慢。`madvise(MADV_WILLNEED)` 是 **async hint、不阻塞、不保證載入完成**——載越多、有更多 async readahead 還在 transit、OS 來不及在第一筆 query 之前 finish。Syscall 本身的 overhead（≈14 µs）可忽略。
...
太多（N=92）**N 個 `MADV_WILLNEED` 啟動的 async readahead 來不及在 first-q
之前完成**——N 越大、有更多 page 還在 transit。Syscall overhead 本身（≈14 µs）
可忽略，**不是**「syscall 開銷吃掉收益」。**N=5** 是 coverage 與 async I/O
完成時間的最佳折衷。
