...
> [第十九維](#第十九維--2c-layers_n-全-n192-dense-sweeprigorous-補測clean--churn--abc--3-layouts)（**2c dense N=0..92**：把每個 (workload × layout) 的 N-sweep 補成全 93 值 × 3 reps，**~5,580 額外 benchmark**；證實 sparse 6-pt 在 9/12 cells 結論正確，但 **A×1b 漏掉 N=62、B×1c 漏掉 N=26、C×1b 漏掉 N=87 三個 sweet spot**）。
...
### 18.2 B2 — A 與 B × layers_N × churn（ck001-010 平均 first-q µs）

| N | A avg first-q | A Δ vs N=0 | B avg first-q | B Δ vs N=0 |
|---:|---:|---:|---:|---:|
| 0（baseline） | 281.40 | — | 499.87 | — |
| 1 | 255.75 | -9.1% | 494.30 | -1.1% |
| **5** | **26.13** | **-90.7%** ← A plateau 起點 | **270.57** | **-45.9%** ← B 接近 plateau |
| 10 | 24.72 | -91.2% | 280.19 | -43.9% |
| 20 | 25.28 | -91.0% | 277.49 | -44.5% |
| 46 | 26.95 | -90.4% | 270.47 | -45.9% |
| **92** | **24.24** | **-91.4%** | **254.05** | **-49.2%** |

跨三 workload 的 layers_N × churn 對照：

| Workload | leaf 是否自然熱 | layers_5 Δ | layers_92 Δ | 絕對 avg @ layers_92 | 來源 |
|---|---|---:|---:|---:|---|
| A（Zipfian, [8, 99997]） | ✅ 熱 keys 反覆 hit | **-90.7%** | -91.4% | **24.24 µs** | 本節 |
| **B（Uniform, [1, 99999]）** | ❌ 每筆 cold leaf fault | -45.9% | **-49.2%** | 254.05 µs | 本節 |
| C（high-key, [590k, 610k]） | ❌ 同上 | -10.6% | **-53.9%** | 213 µs | 第十維 |
...
## 第十九維 — 2c layers_N 全 N=1..92 dense sweep（rigorous 補測：clean + churn × A/B/C × 3 layouts）

### 為什麼有這一維

前幾維的 2c layers_N 結果都是**只在 6 個 N 上採樣**（N ∈ {1, 5, 10, 20, 46, 92}）。
這夠看出 U-shape 跟 plateau 的形狀，但**無法確認真正的最佳 N**——尤其在中段
（N=15..45）跟尾段（N=50..91）兩個區段裡，可能有 sparse 取樣漏掉的局部最小。
本維把每個 (workload × layout) 的 N-sweep 補成 **N=0..92 全 93 個值**，跑 3 reps
取 median，產出兩組高解析度數據：

1. **Clean DB**：A/B/C × {1a orig, 1b vacuum, 1c type-aware} × N=0..92 × 3 reps =
   **2,511 cells**，~10 分鐘 wallclock
2. **Churn DB**：A/B/C × N=0..92 × 11 checkpoints（baseline + ck001..010 × 50k churn ops 累積）
   = **3,069 benchmark runs**，~23 分鐘 wallclock

### Harness

跟第十二維 / 第十八維**一模一樣**（同 `prefetch_layers` binary、同 `posix_fadvise(DONTNEED)`
evict、同 benchmark_harness、同 reference DB），只是把 N grid 從 6 個展開成 93 個。
單 cell 約 0.26s（clean）或 5-6s（churn，含 build work-db + 10 次 5k churn ops）。

### 19.1 Clean DB — dense vs sparse 對照（best N per cell）

3 reps median。「sparse best」= 從 6 個原本採樣點裡找最低；「dense best」=
從 93 個 N 全找最低。

| Workload × Layout | Sparse best (舊) | Dense best (新) | Baseline | Dense vs sparse |
|---|---|---|---:|---:|
| A × 1a orig | N=10 → 293.8 µs (-41.8%) | N=11 → 290.9 µs (-42.4%) | 504.7 µs | **−1.0%** ≈ 同 |
| A × 1b vacuum | N=5 → 478.8 µs (-21.1%) | **N=62 → 420.4 µs (-30.7%)** | 606.9 µs | **−12.2%** ⭐ |
| A × 1c ta | N=92 → 156.5 µs (-75.3%) | N=91 → 153.6 µs (-75.8%) | 633.5 µs | **−1.9%** |
| B × 1a orig | N=20 → 350.5 µs (-51.9%) | N=44 → 337.8 µs (-53.7%) | 729.3 µs | −3.6% |
| B × 1b vacuum | N=5 → 438.1 µs (-53.2%) | N=26 → 431.8 µs (-53.9%) | 935.9 µs | −1.4% |
| B × 1c ta | N=46 → 530.9 µs (-27.3%) | **N=26 → 467.3 µs (-36.0%)** | 730.2 µs | **−12.0%** ⭐ |
| C × 1a orig | N=92 → 596.2 µs (-44.8%) | N=92 → 596.2 µs (-44.8%) | 1,079.3 µs | 0% ✅ |
| C × 1b vacuum | N=92 → 428.0 µs (-53.0%) | **N=87 → 392.2 µs (-57.0%)** | 911.4 µs | **−8.4%** |
| C × 1c ta | N=92 → 370.4 µs (-50.4%) | N=92 → 370.4 µs (-50.4%) | 746.3 µs | 0% ✅ |

### 19.2 Churn DB — dense vs sparse 對照（10-checkpoint avg first-q）

| Workload | Sparse best | Dense best | Baseline | Dense vs sparse |
|---|---|---|---:|---:|
| A × churn | N=10 → 20.7 µs (-92.9%) | N=19 → 16.1 µs (-94.5%) | 290.5 µs | −22.4%（絕對 4.6 µs，雜訊內）|
| B × churn | N=20 → 254.8 µs (-51.3%) | N=25 → 247.9 µs (-52.7%) | 523.6 µs | −2.7% |
| C × churn | N=92 → 208.2 µs (-58.2%) | N=92 → 208.2 µs (-58.2%) | 497.8 µs | 0% ✅ |

### 19.3 主要發現

1. **Sparse 6-pt 在大多數 cell 已經找到正確 plateau 區段**（A/B/C × 1a × clean 跟所有
   C 都是 sparse 找對的；7/12 cells 的 dense gain 在 ±5% 雜訊內）。原本 sparse
   sampling 的結論**不需要修正**。
2. **3 個 cell 有顯著遺漏的 sweet spot**（dense gain ≥8%）：
   - **A × 1b vacuum**：sparse 報的 N=5 (-21%) 不是最佳；**N=62 −31%** 才是。
     1b 的 interior offset 分布比 1a 更散，sparse 採樣完全跳過了 N=46..92 中段
     的下凹區。
   - **B × 1c ta**：sparse 報 N=46 (-27%)，dense 找到 **N=26 −36%**。Type-aware
     layout 把 interior 集中到檔頭後，B 的 cold-leaf fault 是上限，N>26 反而
     開始浪費 syscall。
   - **C × 1b vacuum**：sparse N=92 (-53%)，dense N=87 (-57%)；最後幾個 N
     反而退化，因為 VACUUM 把整檔壓縮後，最尾端的 interior page 不在 hot path 上。
3. **3 個 cell 的「最佳 N」其實是雜訊**：A × 1a/1c clean、A × churn 的 dense
   best 跟 sparse best 差 1-22%（µs 絕對差 1.5-4.6），都在 3-rep noise 範圍內。
   報告時應該講「N≥5 plateau」而不是釘住某個特定 N。
4. **U-shape 的內部不是平的**：A × 1a 在 N=4..15 區間實測在 290-345 µs 之間
   小幅振盪（圖 11），不是平坦 plateau；雜訊+真實 micro-variation 都有。
   sparse sampling 只看 N=5/10 容易誤判成「N=5 sharp minimum」。
5. **Churn N-sweep 的 plateau 寬度跟乾淨 DB 完全一致**——A churn 是 N≥4 plateau
   −93~95%，B churn 是 N=20..30 plateau −51~53%，C churn 在 N=92 才到 −58%。
   churn 不改變 plateau 形狀的結論（第七 / 十維 / 十八維）在 dense 採樣下**再次確認**。
6. **嚴謹度提升的代價**：83 個額外的 N 值 × 3 layouts × 3 workloads × 3 reps
   = 2,241 額外 benchmark 跑了 ~10 分鐘；churn 補測 78 個 × 3 × 11 = 2,574 額外
   benchmark 跑了 ~23 分鐘。**總計 ~33 分鐘 wallclock 換到 +5pp 邊際改善的可
   能性 + 雜訊 vs sweet spot 區分**。

### 19.4 對既有報告的影響

- **README.md 第十二維說「A 上 layers_5 是甜蜜點」** → 仍正確（在 1a × clean），
  但 **1b vacuum 上 N=62 才是真正最佳**（不是 N=5），這點 sparse 漏掉。實務建議
  改成「A × 1a clean 用 N=5/10，A × 1b vacuum 想要極致就用 N=60..80」。
- **第八維 / 第十維 / 第十二維 / 第十八維的 N-sweep 結論** → 全部不需要改，
  只是粒度更高的確認。
- **REPORT.md 的「prefetch 前 5 個 interior」表述** → 仍然是合理的 cost-effective
  預設（5 syscall 拿 −42% 改善），sparse 沒漏掉這個推薦點。

### 圖表

- **Figure 11**（clean DB 3×3 grid）：[figures/out/11_nsweep_full.png](figures/out/11_nsweep_full.png)
  ——把 dense 1..92 跟 sparse 6-pt 疊在一張圖，三 layout × 三 workload 全覆蓋
- **Figure 12**（churn 三 workload）：[figures/out/12_nsweep_full_churn.png](figures/out/12_nsweep_full_churn.png)

### 資料來源

- Clean DB driver: [layout_rewriter/runs/runmatrix_Nsweep_FULL.sh](layout_rewriter/runs/runmatrix_Nsweep_FULL.sh)
- Clean DB CSVs: [layout_rewriter/runs/nsweep_full/full_{orig,vacuum,ta}.csv](layout_rewriter/runs/nsweep_full/)
- Churn drivers: [prefetch_churn/runs_nsweep_full_{a,b,c}/run_full_{a,b,c}.sh](prefetch_churn/)
- Churn matrices: `prefetch_churn/runs_nsweep_full_{a,b,c}/matrix_full_churn_{first_q_us, avg_per_N}.csv`
- Aggregate: [prefetch_churn/runs_nsweep_full_a/aggregate.py](prefetch_churn/runs_nsweep_full_a/aggregate.py)
- Figure scripts: [figures/11_nsweep_full.py](figures/11_nsweep_full.py), [figures/12_nsweep_full_churn.py](figures/12_nsweep_full_churn.py)
