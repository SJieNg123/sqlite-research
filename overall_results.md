...
**規則:**
- 🧭 **跨表比較一律用相對量,不用絕對 µs** —— 這是本報告**唯一**的橫向比較軸。不同批之間不只機器狀態不同,連**跑的 DB / workload 都可能不同**(native = 真 `test.db` 26331 頁、合成 A/B/C = 另一顆生成 DB、`size_1gb` = 1GB 的第三顆),不同 DB 幾何 → 絕對延遲**天生就是不同尺度**。所以絕對 µs 跨批看起來不一樣是**預期且正確**的,不代表資料不一致;要跨批比就比相對量。
...
- ⚠️ **絕對 µs 跨群勿逐格對** —— 同一個量測 `layers_92 A/orig` 在 master(群③)= **393µs**、在 nsweep(群①)= **333µs**,
  15% 差**純機器狀態**;`z`(群②)整列比 A/B/C(群③)低 ~6%(機器狀態);`size_1gb`(群④)比 master 低 ~25%(**機器狀態＋另一顆 DB 的幾何**,見 DB 尺寸章)。這些都不是策略/workload 的真效應。
...
- workstation 上**強效(R≥0.30)的 41 個 cell,全部 41 個在 OpenWhisk 同樣有效(41/41)**。
- Spearman 秩相關 ρ ≈ **0.76(全 55)/ 0.79(高信心)/ 0.81(高信心且非位置敏感)**;方向一致 **46/55**;|R 差| 中位數 **0.112**。
- **唯一在 OpenWhisk 讀作有害的 cell 是 `YCh01/layers_5`**(R_ws≈+0.025 中性 → R_ow −0.596)——是 workstation 上本就中性的策略、非強效策略失效;其餘 8 個不一致落在 ±10% 中性邊界附近。
- **上表 55 格凍結為「修訂後凍結」(revised freeze),是唯一對外的凍結來源。** 其中 7 格(第六 campaign 5 格、第七 campaign 2 格)採用針對性、獨立重建、**嚴格位置平衡**的複現估計,其餘 48 格逐位元沿用歷史值;**歷史凍結表逐位元完整保存**於 `effectiveness_ow_vs_workstation_historical_freeze.csv`、逐格取代紀錄於 `effectiveness_freeze_revision.json`(含 old/new R_ow、campaign、pair 數、位置平衡、SHA256),修訂只改變這 7 格採用哪個已稽核估計、不改變覆蓋(仍 65/65)。三個原本讀作 harmful 的強效 cell(`C/2d` +0.432、`C/layers_92` +0.383、`C_hit/2e_K40` +0.487)在平衡複現下皆翻回正向,故無強效策略在 OpenWhisk 有害。其中 **4 格 position-sensitive**(`C/layers_92`、`C/2d`、`YCu/layers_5`、`YCh01/2f_top14`):平衡總計為正、但兩位置子集符號不一致,屬**描述性平衡批次估計**、非乾淨的位置無關因果效應(歸因於 pair-position / 短暫執行—儲存狀態,**不**歸因 page-cache carryover),已排除於 ρ≈0.81 的乾淨子集。七個 OpenWhisk campaign 各司其職、**永不 pool**(5756 invocation / 2878 pair 為 unpooled 記帳、非單一估計)。
- lp 那側:shuffle 交付在**兩個平台**都比 sorted 慢(order_ratio 均 >1),交付後首查維持在數 µs 內(對照組)——交付順序這個機制與其成本結構同樣可攜。

這是一個**描述性的跨平台一致性**結果——**不**主張絕對延遲相等、效果量相等、因果等價、或硬體無關的加速倍率。逐 cell 表與稽核見 `deployment/openwhisk/analysis/comparison/VERDICT_effectiveness_portability.md`、修訂後凍結 `effectiveness_ow_vs_workstation_revised_freeze.csv`(歷史 `effectiveness_ow_vs_workstation_historical_freeze.csv`、取代紀錄 `effectiveness_freeze_revision.json`)、`lp_delivery_order.csv`;full-matrix 覆蓋稽核見 `analysis/comparison/workstation_canonical_matrix.csv`(65/65 BOTH、0 WS_ONLY、4 OW_ONLY)。
