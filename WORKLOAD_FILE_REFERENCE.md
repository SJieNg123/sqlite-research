...
|---|---|---:|
| 1a 原始 | `pipeline/preparation/layout_rewriter/runs/test.db` | 0.96 |
| 1b VACUUM | `pipeline/preparation/layout_rewriter/runs/test_vacuum.db` | 1.13 |
| 1c type-aware | `pipeline/preparation/layout_rewriter/runs/test_typeaware.db` | 0.0001 |
...
`strategies/access/runs/test*.db` 跟 `strategies/slru/runs/test*.db` 都是 symlink 指到上面三個。

### 1.2 三個 classify.csv（page_number → page_type 的對照，每個 DB 一份）

| Layout | File |
|---|---|
| 1a | `pipeline/preparation/layout_rewriter/runs/classify_before.csv` |
| 1b | `pipeline/preparation/layout_rewriter/runs/classify_vacuum.csv` |
| 1c | `pipeline/preparation/layout_rewriter/runs/classify_after.csv` |

`strategies/access/runs/classify_*.csv` 也是 symlink。

### 1.3 三個 cold script（per-layout cold-start drop）

**2026-06-19 起全部走 §0**：所有 cold script 統一 `exec /usr/local/sbin/drop-caches`
（全機 page cache + dentries + inodes 一刀清）。layout 不再決定 evict 機制——
這三個檔留著只是為了 orchestrator 向後相容（各自的路徑不變）。

| Layout | File | 做什麼 |
|---|---|---|
| 1a | `pipeline/preparation/layout_rewriter/runs/cold_orig.sh` | `exec /usr/local/sbin/drop-caches` |
| 1b | `pipeline/preparation/layout_rewriter/runs/cold_vacuum.sh` | `exec /usr/local/sbin/drop-caches` |
| 1c | `pipeline/preparation/layout_rewriter/runs/cold_ta.sh` | `exec /usr/local/sbin/drop-caches` |
...
| `prefetch_access` | [`strategies/access/src/prefetch_access.c`](strategies/access/src/prefetch_access.c) | **2d/2e**（history + page-type aware）+ **3a/3b** ratio variant | `<db> <classify.csv> <hotpages.csv> <cap_interior> <cap_leaf> <page_size>` |
| `prefetch_slru` | [`strategies/slru/src/prefetch_slru.c`](strategies/slru/src/prefetch_slru.c) | **2f SLRU** | `<db> <hotpages.csv> <page_size>` |
...
| Tool | Source | 用途 |
|---|---|---|
| `benchmark_harness` | [`pipeline/engine/benchmark_harness/benchmark_harness.c`](pipeline/engine/benchmark_harness/benchmark_harness.c) | 跑一格 cold-start latency measurement |
| `gen_hotleaves.py` | [`strategies/access/runs/gen_hotleaves.py`](strategies/access/runs/gen_hotleaves.py) | 從 (DB, classify, base hotpages, workload, K) 產 hot2e_*.csv |
...
| 用途 | File |
|---|---|
| Workload .txt | `workloads/workload_a.txt`（原名 `workloadc.txt`，2026-06 改名以跟 B/C 對齊）|
| | 同檔的 symlink：`strategies/slru/runs/workload_a_zipfian.txt`、`strategies/access/runs/workload_a_zipfian.txt`、`prefetch_churn/generated_workloads/workload_a_zipfian.txt` |
| Base hotpages（1a/1b/1c）| `strategies/access/runs/hotpages_a{,_vacuum,_ta}.csv` |
| hot2e（18 個檔）| `strategies/access/runs/hot2e_A_{orig,vacuum,ta}_K{10,40,50,92,100,500}.csv` |

### 2.2 Workload B（uniform, keys [1, 99999]）

| 用途 | File |
|---|---|
| Workload .txt | `workloads/workload_b.txt` |
| | 另一份 copy: `strategies/access/runs/workload_b_uniform.txt` |
| Base hotpages | `strategies/access/runs/hotpages_b{,_vacuum,_ta}.csv` |
| hot2e | `strategies/access/runs/hot2e_B_{orig,vacuum,ta}_K{10,40,50,92,100,500}.csv` |

### 2.3 Workload C（high-key, keys [590k, 610k]）

| 用途 | File |
|---|---|
| Workload .txt | `workloads/workload_c.txt` |
| | 另一份 copy: `strategies/access/runs/workload_c_highkey.txt` |
| Base hotpages | `strategies/access/runs/hotpages_c{,_vacuum,_ta}.csv` |
| hot2e | `strategies/access/runs/hot2e_C_{orig,vacuum,ta}_K{10,40,50,92,100,500}.csv` |

### 2.4 Workload Z（Zipfian low-key, [1, 1000]）— robustness 對照

| 用途 | File |
|---|---|
| Workload .txt | `workloads/workload_z.txt` |
| Generator | `workloads/gen_zipf_lowkey.py` |
...
| 用途 | File |
|---|---|
| Write workload .txt | `workloads/workload_churn_write.txt` |
...
## 3. 一格 benchmark 的精確 command（**統一 pipeline 版本**）

實務上**整個矩陣由 [`run_experiment.py`](run_experiment.py) 跑**（雙臂 + baseline + 凍結 + 彙整一次到位）。
單格底層 command 長這樣（④ 的驗證已**內建進 harness**，不再有外部步驟）：

```sh
pipeline/engine/benchmark_harness/benchmark_harness \
  ...
  --post-cold-script   <deliver.sh: WARM_METHOD=pread|fadvise warmer db hotset 4096> \  # ③（統一 warmer 引擎）
  --verify-hotset      <hotset.csv> \                     # ④（內建 cold_pct + delivery_pct，無外部 residency_checker）
...
統一 pipeline 下**統一改走 `warmer`**，native tool 降為離線 hotset 產生器。歷史簽章（仍可離線用）：
...
[`pipeline/engine/benchmark_harness/BENCHMARK_HARNESS.md`](pipeline/engine/benchmark_harness/BENCHMARK_HARNESS.md)。
...
pipeline/engine/benchmark_harness/benchmark_harness \
  ...
  --cold-advice dontneed \
  --drop-caches-script pipeline/preparation/layout_rewriter/runs/cold_orig.sh
# 跑完之後用 residency_checker 拍快照
pipeline/engine/residency_checker/residency_checker \
  pipeline/preparation/layout_rewriter/runs/test.db \
  strategies/access/runs/hotpages_w.csv
```
...
for layout in orig vacuum ta; do
  case $layout in
    orig)   db=pipeline/preparation/layout_rewriter/runs/test.db;          cl=classify_before.csv;  hot_suffix= ;;
    vacuum) db=pipeline/preparation/layout_rewriter/runs/test_vacuum.db;   cl=classify_vacuum.csv;  hot_suffix=_vacuum ;;
    ta)     db=pipeline/preparation/layout_rewriter/runs/test_typeaware.db; cl=classify_after.csv;   hot_suffix=_ta ;;
  esac
  for K in 10 40 50 92 100 500; do
    python3 strategies/access/runs/gen_hotleaves.py \
      $db \
      pipeline/preparation/layout_rewriter/runs/$cl \
      strategies/access/runs/hotpages_w$hot_suffix.csv \
      new_workloads/workload_W.txt \
      $K \
      strategies/access/runs/hot2e_W_${layout}_K${K}.csv
  done
done
```
...
| Driver | 包含 |
|---|---|
| [`strategies/access/runs/runmatrix_2d.sh`](strategies/access/runs/runmatrix_2d.sh) | 2d × A/B/C × 3 layout × 3 reps |
| [`strategies/access/runs/runmatrix_2e_abc.sh`](strategies/access/runs/runmatrix_2e_abc.sh) | 2e_K* × A/B/C × 3 layout × 6 reps |
| [`strategies/access/runs/runmatrix_ram_pressure_full.sh`](strategies/access/runs/runmatrix_ram_pressure_full.sh) | A/B/C × 1a/1b/1c × 7 策略 × 2 mem × 6 reps（756 cells）|
| [`pipeline/preparation/layout_rewriter/runs/runmatrix_Nsweep_FULL.sh`](pipeline/preparation/layout_rewriter/runs/runmatrix_Nsweep_FULL.sh) | 2c × N=0..92 × A/B/C × 3 layout × 3 reps |
...
|---|---|---|
| Clean-DB matrix | `pipeline/preparation/layout_rewriter/runs/matrix_*_results.csv` | `matrix_W_results.csv` |
| Access-pattern matrix | `strategies/access/runs/matrix_2{d,e}_results.csv` | 新欄位加 workload column |
...
不確定就翻 [`overall_strategies.md`](overall_strategies.md) 看每個策略原理，或翻 [`pipeline/engine/benchmark_harness/BENCHMARK_HARNESS.md`](pipeline/engine/benchmark_harness/BENCHMARK_HARNESS.md) 看 harness 細節。
