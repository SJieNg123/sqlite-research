...
- **L2 tree-top 暖法（最該先做）**：只同步暖 root + 上 1–2 層（頁數從 92 砍到個位數 → warm 成本從 8 ms 砍到 µs 級），
  預期能**同時拿到 pread 的 −77% 與接近 fadvise 的低成本**，直接解掉發現②的淨虧問題。需先算每頁 tree level。
- **L3/L4 線上語意預取（pointer-ahead / fan-out）**：邊走訪 B-tree 邊把下一層的 child 非同步預取出來；要 hook
  SQLite 讀路徑（VFS xRead），比 batch warmer 侵入。
