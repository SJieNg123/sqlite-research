#!/usr/bin/env python3
"""Descriptive coverage for the OpenWhisk cross-workload PORTABILITY campaign (§12).

Reads the frozen portability normalized tables and emits three DESCRIPTIVE CSVs +
a manifest. Descriptive only: coverage grid, plan parity, per-workload summary.
No speedup, ratio, ranking, winner, or latency comparison is computed or implied.
Native/WK1 remains the primary performance evidence; this is a deployment /
cross-workload portability complement.

Outputs (under --out, default deployment/openwhisk/analysis/descriptive/portability/):
  portability_coverage.csv          one row per executed (block, workload, strategy, seed, handle_mode) cell
  portability_plan_parity.csv       one row per executed target plan (parity_type per §8)
  portability_workload_summary.csv  one row per workload family
  portability_descriptive_manifest.json
"""
import argparse
import csv
import hashlib
...
from normalize_portability import (                             # noqa: E402
    PORTABILITY, parity_type, load_freeze_index,
)

SCHEMA_VERSION = 1

# workload_id -> short family code (§15 five representative families)
WORKLOAD_FAMILY = {
...
def build_coverage(pairs):
    """One row per executed (block, workload, strategy, seed, handle_mode) cell."""
...
def build_plan_parity(invocations, freeze_idx):
    """One row per distinct executed target plan (strategy, workload, seed)."""
    seen = {}
    for r in invocations:
        ...
        if key in seen:
            continue
        static = strat in PORTABILITY["static_strategies"]
...
def run(ow_root, norm_dir, out_dir):
    ...
    invocations = _read(os.path.join(norm_dir, "portability_normalized_invocations.csv"))
    pairs = _read(os.path.join(norm_dir, "portability_normalized_pairs.csv"))
    ...
    cov_sha = _sha_and_write(os.path.join(out_dir, "portability_coverage.csv"), cov_cols, cov)
    par_sha = _sha_and_write(os.path.join(out_dir, "portability_plan_parity.csv"), parity_cols, parity)
    ws_sha = _sha_and_write(os.path.join(out_dir, "portability_workload_summary.csv"), wsum_cols, wsum)
    ...
    manifest = {
        ...
        "inputs": {
            "portability_normalized_invocations.csv": len(invocations),
            "portability_normalized_pairs.csv": len(pairs),
        },
        ...
        "workloads": len(wsum),
        "outputs": {
            "portability_coverage.csv": {"rows": len(cov), "sha256": cov_sha},
            "portability_plan_parity.csv": {"rows": len(parity), "sha256": par_sha},
            "portability_workload_summary.csv": {"rows": len(wsum), "sha256": ws_sha},
        },
    }
    with open(os.path.join(out_dir, "portability_descriptive_manifest.json"), "w") as f:
...
def main():
    ...
    ap.add_argument("--norm", default=str(_OW_ROOT / "analysis" / "normalized" / "portability"))
    ap.add_argument("--out", default=str(_OW_ROOT / "analysis" / "descriptive" / "portability"))
    a = ap.parse_args()
    m = run(a.ow_root, a.norm, a.out)
    print("portability descriptive: %d coverage cells, %d target plans, %d workloads -> %s"
