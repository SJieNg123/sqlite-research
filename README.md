...
- ~~**access-pattern × A 的 delete-heavy churn 會不會 decay？**~~ → **✅ 已完成（B1，gap audit）**：3 arms × 10 checkpoint。**2e_K10_static -92.4%, 2d_static -91.8%, 2e_K50_static -91.5%**——全部贏過 layers_92 (-91.4%)，hypothesis（delete from id=1 會擾動 Zipfian 低 id 熱 keys）**被推翻**。原因：50k delete 集中在窄區、B+tree 不立即 merge、hot leaves 持續被 read 命中。**access-pattern × static hot 在 insert/delete 兩種正交 churn 下都穩定**。詳見 [prefetch_churn/runs_access_churn_a/README.md](prefetch_churn/runs_access_churn_a/README.md) 以及 [overall_results.md 第十八維](overall_results.md#第十八維--churn-擴充abc--churn--2c-layers_n--2d--2e_kab--churn--statictk0-hotpages)
- ~~**layers_N × churn 換成 A / B workload，plateau 形狀會不會變？**~~ → **✅ 已完成（B2，gap audit）**：A × churn N=5 已到 plateau (-90.7%)、N=92 (-91.4%)；B × churn N=5 接近 plateau (-45.9%)、N=92 (-49.2%)。**Churn 不改變 layers_N 的 plateau 形狀**——A 是 leaf 自然熱、interior 是唯一瓶頸（N=5 就夠）；B/C 是 cold leaf workload、上限卡在 -50% 左右。詳見 [prefetch_churn/runs_nsweep_a/README.md](prefetch_churn/runs_nsweep_a/README.md) + [prefetch_churn/runs_nsweep_b/README.md](prefetch_churn/runs_nsweep_b/README.md)
- ~~**B × access-pattern × churn：uniform reads 沒有自然熱葉，access-count 排出的 top-K leaves 會不會退化成隨機選頁、不帶效益或失效？**~~ → **✅ 已完成（B3，audit 最後一塊缺口）**：2d_static **-45.7%** / 2e_K10_static **-48.8%** / 2e_K50_static **-47.7%**，跟 file-offset 的 layers_5 (-45.9%) / layers_92 (-49.2%) **打平**——top-K leaves 對 uniform reads 等同隨機選頁、沒帶來額外效益；但 ck001→ck010 沒有單調惡化，**static t=0 hot 在 B 上也不 decay，只是不帶效益、不是會壞掉**。B 的 ~-49% 天花板由 cold-leaf fault 鎖死、不由 prefetch 策略決定。詳見 [prefetch_churn/runs_access_churn_b/README.md](prefetch_churn/runs_access_churn_b/README.md) 以及 [overall_results.md 第十八維 18.3](overall_results.md#第十八維--churn-擴充abc--churn--2c-layers_n--2d--2e_kab--churn--statictk0-hotpages)
- ~~**2c layers_N sparse 6-pt（N=1/5/10/20/46/92）真的是全部 cell 的最佳 N 嗎？**~~ → **✅ 已完成（第十九維，rigor 補測）**：把每個 (workload × layout) 的 N-sweep 補成 **N=0..92 全 93 個值 × 3 reps**，clean DB 跑 2,511 cells、churn DB 跑 3,069 cells，總共 **~5,580 額外 benchmark**。**9/12 cell 的 sparse 結論正確**，但有 3 個 cell 漏掉真正的 sweet spot：**A × 1b vacuum 在 N=62 才是 -31%（sparse 找 N=5 只有 -21%，漏 12pp）、B × 1c ta 在 N=26 才是 -36%（sparse 找 N=46 -27%，漏 12pp）、C × 1b vacuum 在 N=87 才是 -57%（sparse 找 N=92 -53%，漏 8pp）**。U-shape 也不是 sharp、是雜訊偏多的寬 plateau（A × 1a 在 N=4..15 區間 290-345 µs 雜訊振盪）。Churn N-sweep plateau 形狀跟 clean DB 完全一致 → churn 不改變 plateau 形狀的結論在 dense 採樣下再次確認。詳見 [overall_results.md 第十九維](overall_results.md#第十九維--2c-layers_n-全-n192-dense-sweeprigorous-補測clean--churn--abc--3-layouts) + [Figure 11](figures/out/11_nsweep_full.png) / [Figure 12](figures/out/12_nsweep_full_churn.png)
...
4. **2d/2e access-pattern prefetch 在 churned DB 上會不會退化？** → **在 C × insert / A × delete / B × churn 三個正交設置下都不會退化**。10 checkpoints × 50k 寫入 ops，**靜態 hotpages（t=0 計算）** 在四組設置都撐住：(a) **C × insert-churn**: acc_2e_K10 -91% / acc_2d -50%（hot leaves 跟 insert target 重疊）；(b) **A × delete-heavy churn (B1 補測)**: 2e_K10_static **-92.4%** / 2d_static **-91.8%** / 2e_K50_static -91.5%，全部贏過 layers_92（-91.4%）。原假設「delete from id=1 會擾動 A 的低 id 熱 keys」被推翻——50k delete 集中在窄區、B+tree 不立即 merge、hot leaves 持續被 read 命中。(c) **A/B × layers_N × churn (B2 補測)**: A layers_5 -90.7% / B layers_5 -45.9%，churn 不改變 N-sweep plateau 形狀。(d) **B × access-pattern × churn (B3 補測)**: 2d -45.7% / 2e_K10 **-48.8%** / 2e_K50 -47.7%，跟 file-offset 的 layers_5 (-45.9%) / layers_92 (-49.2%) **打平**——uniform reads 沒有自然熱葉，top-K leaves 等同隨機選頁、沒帶來額外效益，但也不 decay。**結論**：access-pattern × static hot 是 production-ready baseline；plateau 高度由 workload leaf-warmth 決定（Zipfian -91% / uniform/high-key -49~54%），不由 churn 決定；B 上 access-pattern vs file-offset 沒差別、A/C 上 access-pattern 略勝。
