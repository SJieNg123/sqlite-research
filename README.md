...
### 第 1 章：先看清楚地形 — `pipeline/preparation/classify_pages/`
...
### 第 2 章：建一台儀器 — `pipeline/engine/benchmark_harness/`
...
### 第 9 章：救回來了 — Type-aware VACUUM（`pipeline/preparation/layout_rewriter/`）

第 5 章的 cliffhanger：SQLite 內建 VACUUM 把 prefetch 效益從 -54% 打到 -9%。如果我們**自己寫一個 type-aware 的版本**，把 interior 全部排到檔頭，會發生什麼？

`pipeline/preparation/layout_rewriter/layout_rewriter.c` 直接讀 source DB、重寫一個新 DB，把所有 interior pages 排到 page 2–93（連續），leaves 排在後面。**整個過程不碰 SQLite source code，純粹操作 file format**。
...
> 📂 見 [pipeline/preparation/layout_rewriter/LAYOUT_REWRITER.md](pipeline/preparation/layout_rewriter/LAYOUT_REWRITER.md) 與 [overall_results.md 第二維](overall_results.md#第二維--layout-對-strategy-的放大效果workload-a-only)。

### 第 10 章：偷看 OS 的 cache — 2f SLRU（`strategies/slru/`）
...
> 📂 見 [strategies/slru/PREFETCH_SLRU.md](strategies/slru/PREFETCH_SLRU.md)。
...
實作：跑一遍 workload 拿到「真實被 walk 的 interior pages + 真實 hottest leaves」（`strategies/access/`），然後 cold start 時 madvise 那些 page。**這需要先有一輪 access history**——所以是 warm-start 策略，不是 first-ever-launch 策略。
...
1. **2d/2e access-pattern prefetch 能否在 Workload C 上以 <10 個 syscall 達到接近 layers_92 的 -46% 改善？**
   ...
   資料來源：[overall_results.md 第十四維](overall_results.md#第十四維--2d-access-pattern-prefetch-interior-only--abc--3-layouts)、[第十五維](overall_results.md#第十五維--2e-access-pattern-prefetch-interior--top-k-leaves--abc--3-layouts--kk10k50k100k500)；matrix CSV [strategies/access/runs/matrix_2d_results.csv](strategies/access/runs/matrix_2d_results.csv) + [matrix_2e_results.csv](strategies/access/runs/matrix_2e_results.csv)；圖 [figures/out/05_strategy_comparison.png](figures/out/05_strategy_comparison.png)。
...
2. **2f SLRU 在 RAM 緊（cgroup MemoryMax=20M）時還有用嗎？**
   ...
   資料來源：[overall_results.md 第十六維](overall_results.md#第十六維--ram-pressure-完整矩陣cgroup-memorymax20-mb-abc--1a1b1c--base-2d-2e_k1050100500-2f_slru)；matrix CSV [strategies/access/runs/matrix_ram_full_results.csv](strategies/access/runs/matrix_ram_full_results.csv)（756 cells × 6 reps）；圖 [figures/out/06_ram_pressure_heatmap.png](figures/out/06_ram_pressure_heatmap.png)（✅ `run_experiment.py --mem-limit 20M` vs unlimited）。
...
3. **多 process 場景下，prefetch worker cadence 該多大？**
   ...
   資料來源：[multiprocess/runs_prefetch_cadence/README.md](multiprocess/runs_prefetch_cadence/README.md)；matrix CSV [multiprocess/runs_prefetch_cadence/cadence_results.csv](multiprocess/runs_prefetch_cadence/cadence_results.csv)；圖 [figures/out/08_cadence_comparison.png](figures/out/08_cadence_comparison.png)（✅ `cadence.py`，背景 warmer + 全機 drop-caches probe）。
...
├── pipeline/preparation/classify_pages/         # SQLite page-type 分類器
├── pipeline/engine/benchmark_harness/      # Cold-start workload benchmark 工具
├── pipeline/engine/residency_checker/      # Page residency 檢查工具
...
├── pipeline/preparation/layout_rewriter/        # ⭐ Type-aware layout rewriter + cross-layout 矩陣（第 9, 11 章）
├── strategies/slru/          # ⭐ 2f SLRU prefetch via mincore（第 10 章）
├── strategies/access/        # ⭐ 2d/2e access-pattern prefetch + 756-cell RAM-pressure 矩陣（第 12 章）
...
| 目錄 | 章節 | 角色 |
|------|------|------|
| `pipeline/preparation/classify_pages/` | 第 1 章 | 看清 DB 內部結構的基礎工具 |
| `pipeline/engine/benchmark_harness/` | 第 2 章 | 測量儀器 |
| `pipeline/engine/residency_checker/` | 第 2 章 | 輔助 residency 量測 |
...
| `pipeline/preparation/layout_rewriter/` | 第 9, 11 章 | Type-aware layout + cross-workload 對照矩陣 |
| `strategies/slru/` | 第 10 章 | mincore-based working-set preload |
| `strategies/access/` | 第 12 章 | Access-pattern prefetch (2d/2e) + 756-cell RAM-pressure 矩陣 |
| `frontend/16week_plan.jsx` | — | 整個研究計畫的 UI tracker |

## 各實驗目錄

### [pipeline/preparation/classify_pages/](pipeline/preparation/classify_pages/) — SQLite Page Classifier
...
```bash
gcc -O2 -Wall -o classify_pages pipeline/preparation/classify_pages/classify_pages.c
python3 pipeline/preparation/classify_pages/build_testdb.py
./classify_pages test.db > pages.csv 2> stats.txt
python3 pipeline/preparation/classify_pages/plot_pages.py pages.csv page_layout.png
```

### [pipeline/engine/benchmark_harness/](pipeline/engine/benchmark_harness/) — Cold-start Benchmark Harness

觀察 SQLite workload 在 cold-start 情境下的 latency / page fault / residency。詳見 [pipeline/engine/benchmark_harness/BENCHMARK_HARNESS.md](pipeline/engine/benchmark_harness/BENCHMARK_HARNESS.md)。
...
- `workloads/workload_a.txt` — 測試用 workload

### [pipeline/engine/residency_checker/](pipeline/engine/residency_checker/) — Residency Checker

檢查 SQLite database 檔案中每個 page 是否 resident。詳見 [pipeline/engine/residency_checker/RESIDENCY_CHECKER.md](pipeline/engine/residency_checker/RESIDENCY_CHECKER.md)。
...
### [pipeline/preparation/layout_rewriter/](pipeline/preparation/layout_rewriter/) — Type-aware Layout Rewriter + 跨 layout/workload 矩陣

從 source DB 直接寫一個 type-aware layout 的新 DB（interior 全排到 page 2-93 連續區），不碰 SQLite source code。包含 cross-workload × cross-layout 完整對照矩陣（第六/七/八/九維）。詳見 [pipeline/preparation/layout_rewriter/LAYOUT_REWRITER.md](pipeline/preparation/layout_rewriter/LAYOUT_REWRITER.md)。

### [strategies/slru/](strategies/slru/) — 2f SLRU Prefetch（mincore-based）

跑完一次 workload 後用 `mincore()` dump 當下 OS page cache 的 residency snapshot，下次 cold start 全部 `madvise(MADV_WILLNEED)`。**完全不用攔截 SQLite 內部**，~70 行 C。詳見 [strategies/slru/PREFETCH_SLRU.md](strategies/slru/PREFETCH_SLRU.md)。

### [strategies/access/](strategies/access/) — Access-pattern Prefetch (2d/2e) + RAM-pressure Matrix

從上一輪 query log 推出 page access count，把 madvise 預算花在「最常被讀的」interior + 最熱 K 個 leaves。2d = interior-only (4-92 syscalls)、2e_K = interior + top-K leaves。**Workload C × 1a 上 2d 只用 4 個 syscall 拿到 -47.6%**（追平 layers_92 的 -46%，syscall 從 92→4 省 23×），**2e_K10 在 C × 任一 layout 用 14-42 syscalls 拿到 -82~84%**（超越 2f SLRU 的 -77%）。包含 756-cell RAM-pressure 完整矩陣（A/B/C × 1a/1b/1c × 7 strategies × {20M, none} × 6 reps）。詳見 [strategies/access/PREFETCH_ACCESS.md](strategies/access/PREFETCH_ACCESS.md)。
...
`pipeline/preparation/classify_pages/plot_pages.py` 對 interior pages 計算 scatter score：
