...
|---|---|---|---|---|
| **Section 5 single-instantiation canonical** | `results/unified_v3/matrix/summary.csv` (+ `raw.csv`, `README.md`) | A/B/C × orig × {baseline, layers_5, 2d, 2e_K10, 2e_K500, 2f_slru}; arms async+pread+baseline; single-instantiation (n=10 reps); one run, one `env.txt`, 2026-09-14 | **supersedes `results/unified_v2` + `results/tiebreak_fix` for every orig single-instantiation claim in Section 5** — Figures 13/14, `tab:e2e-ac`, the `tab:e2e-robustness` single-workload column, the ceiling table and the Section 5 prose. **Not** for layout comparisons (`vacuum`/`ta`), which this batch did not run, and not for any cross-seed claim | Every arm measured in ONE machine state **with the corrected tie-break** (verified by delivered page set, not file hash). Exists because neither prior batch had all five arms at the corrected tie-break, which is what blocked Figure 14 from showing `2e_K*`. Relative values reproduce the prior result with **0 sign flips**; absolute µs are ~1–9% faster and cold open is ~179 µs vs ~230 µs, so **do not mix its absolute values with `unified_v2`**. See `results/unified_v3/README.md`. |
| Main matrix (uncorrected cells) | `results/unified_v2/matrix/summary.csv` (+ `raw.csv`) | A/B/C × {orig,vacuum,ta} × {baseline, layers_5, layers_92, 2d, 2e_K10, 2e_K40, 2e_K92, 2e_K500, 2f_slru}; arms async+pread+baseline; single-instantiation (n=10 reps) | — | Canonical only for cells **not** in the tie-break impact-audit changed set below. **For `orig` single-instantiation, superseded by `results/unified_v3` (row above) since 2026-09-14**; still canonical for the `vacuum`/`ta` layout arms, which `unified_v3` did not run |
| Corrected single-instantiation | `results/tiebreak_fix/master_summary.csv` (+ `master_raw.csv`) | A:`2e_K500`; B:`2e_K10,2e_K40,2e_K92,2e_K500`; C:`2e_K10,2e_K40,2e_K92` — × {orig,vacuum,ta}; single-inst | **supersedes** `results/unified_v2` for exactly these cells | Same-batch baseline + `2f_slru` anchor in the same run. **For `orig`, superseded by `results/unified_v3`**, which measured these arms under the same corrected tie-break alongside `layers_5`/`2d` in one batch; still canonical for `vacuum`/`ta` |
