...
|---|---|---|---|---|
| **Sections 5 and 6 canonical (single-instantiation AND cross-seed), all local axes, no coverage gap** | `results/unified_v6/` — `seed{01..10}/summary.csv`, cross-seed `summary.csv`, CI `uncertainty.csv`, `README.md` | A/B/C/**C_hit** × **orig** × 17 strategies × arms {baseline, pread, async, async_win, async_bulk}; plus A/B/C × **{vacuum, ta, 1gb}** × **13 strategies** (v5's 9 plus the four `2f_topN` budgets) × arms {baseline, pread, async}; seeds 1–10; one **continuous** window 2026-09-23T01:54:17Z → 05:11:34Z, code **`f570d4c`** | **supersedes `results/unified_v5` for every claim**, and through it `unified_v4`, `unified_v2`, `tiebreak_fix`, `results/size_1gb` and the standalone `C_hit` batch. **Not** for the `aging`, `churn`, `cadence` or `ram_pressure` axes, nor OpenWhisk/Lambda | Exists because v5 ran `2f_topN` on `orig` only, leaving `main.tex:553` uncheckable; running those cells as an addendum would have forfeited absolute comparability, so the whole matrix was re-run. 39,390 rows, 10 × 3939, zero failures, `cold_pct` max 0.000, zero exclusions; drift anchor spreads 1.9/0.7/4.6/1.8% (A/B/C/C_hit). **Reproduces v5 on all 376 shared cells, median move 1.8 pt, no substantive sign flip**, so headline claims now rest on two independent batches. Prerequisite inputs from `tools/gen_freqdump_layouts.sh` (360 files, 2m27s, 0 failures). Fixes two v5 defects: the window is **continuous** (v5 spanned a 1.4 h idle gap) and the code was **committed before launch**, so `batch.log`'s HEAD is true. **Key results:** `Dump` at 10× on Tail-Mixed is −9% → **+30%** (the paper's −9% start is exact, its +139% end is not); `Skel+500` fails under both readings (`2e_K500` −29→−32, `Dump-500` −9→−30, both improving); window-chunked delivery leaves us ahead at 0.27×/0.28×/0.62× on A/B/C and **behind at 1.15× on C_hit**; two `main.tex:505` layout claims fail in both batches. **`delivery_pct` is not comparable across arms or layouts** (zero-grace `mincore`). **Gotcha:** `tools/stats_uncertainty.py` must be given seed *directories*, not `raw.csv` globs, or it silently reports `n_seeds=1`. |
| Sections 5 and 6, previous canonical (**superseded 2026-09-23**) | `results/unified_v5/` — `seed{01..10}/summary.csv`, cross-seed `summary.csv`, CI `uncertainty.csv`, `README.md` | A/B/C/**C_hit** × **orig** × 17 strategies (`unified_v4`'s 12 plus `layers_92`, `learned_markov_14`, `learned_markov_28`, `lp_sorted`, `lp_shuf`) × arms {baseline, pread, async, **async_win**, **async_bulk**}; plus A/B/C × **{vacuum, ta, 1gb}** × 9 strategies × arms {baseline, pread, async}; seeds 1–10; one window 2026-09-18T03:03:12Z → 07:14:15Z, code **`fea7c51`** | **supersedes `results/unified_v4` for every `orig` claim**, and additionally supersedes `results/unified_v2` and `results/tiebreak_fix` for the `vacuum`/`ta` and `layers_92` cells they were still canonical for, `results/size_1gb` for the size-scaling paragraph, and the standalone `C_hit` control batch. **Not** for the `aging`, `churn`, `cadence` or `ram_pressure` axes, which use other subcommands and did not run here, and not for OpenWhisk or Lambda | Exists to close the last three provenance seams (`main.tex:505` layout disclaimer, `layers_92` from a foreign batch, `C_hit` in its own batch) **and** to answer the advisor's stronger-`Dump` question. 33,270 rows, 10 × 3327, zero failures, `cold_pct` max 0.000, zero exclusions; drift anchor `2f_slru` spreads 1.5/2.0/2.6/4.1% (A/B/C/C_hit) across the window. Reproduces `unified_v4` seed-for-seed within ~1–3%, no sign flips. **Two new delivery arms:** `async_win` (one `fadvise` per 32-page readahead window) cuts `Dump` delivery 7088→2020 µs and moves it to +147%/+122%/−65.6%/−47.4% on A/B/C/C_hit, beating our best on `C_hit`; `async_bulk` (one uncapped hint per range) delivers only 1.4% and is a real failure. **`batch.log` line 2 says `HEAD: b0edb8c`, which is wrong** — the batch ran uncommitted code later committed verbatim as `fea7c51`. **`delivery_pct` is not comparable across arms** (zero-grace `mincore`); see `results/unified_v5/grace_check/`. |
| Sections 5 and 6, previous canonical (**superseded 2026-09-18**) | `results/unified_v4/` — `seed{01..10}/summary.csv`, CI `uncertainty.csv`, `README.md` | A/B/C × orig × {baseline, layers_5, 2d, 2e_K10, 2e_K500, 2f_slru, 2f_top14, 2f_top28, 2f_top100, 2f_top500, leaf_freq_K10, leaf_rand_K10}; arms async+pread+baseline; seeds 1–10; one contiguous window 2026-09-17T07:23:00Z → 07:52:21Z, code `ee75df1` | **supersedes `results/unified_v3`, `results/ablation_comp_v2`, `results/competitive`, `results/seeds` and `results/tiebreak_fix/seeds` for every orig claim in `tab:e2e-ac`, `tab:ablation`, `tab:competitive`, `tab:seeds`, `tab:ceiling`, Figures 13/14 and the Section 5/6 prose.** **Not** for layout comparisons (`vacuum`/`ta`) or `layers_92`, which this batch did not run, and not for the other axes | Exists because those four tables previously spanned **5 batches and 2 code versions**. One batch, one code version, so all four are mutually comparable. `cold_pct` max 0.000 on all 5940 rows. All 24 printed cells have `delivery_pct_median` = 100.0. Reproduces the prior result with **0 sign flips**; largest move is Tail-Mixed `Dump-500` −13% → −9%, which was batch mixing (it shares a byte-identical 483-page hotset with `Dump`). **Open item:** the cold-open term is strategy-dependent and drifts between batches by ~30 µs, so `e2e_ext` absolutes carry an uncontrolled term. See `results/unified_v4/README.md`. |
| Main matrix (uncorrected cells) | `results/unified_v2/matrix/summary.csv` (+ `raw.csv`) | A/B/C × {orig,vacuum,ta} × {baseline, layers_5, layers_92, 2d, 2e_K10, 2e_K40, 2e_K92, 2e_K500, 2f_slru}; arms async+pread+baseline; single-instantiation (n=10 reps) | — | Canonical only for cells **not** in the tie-break impact-audit changed set below. **For `orig` single-instantiation, superseded by `results/unified_v4` (row above) since 2026-09-17**; still canonical for the `vacuum`/`ta` and `layers_92` arms, which `unified_v4` did not run |
| Corrected single-instantiation | `results/tiebreak_fix/master_summary.csv` (+ `master_raw.csv`) | A:`2e_K500`; B:`2e_K10,2e_K40,2e_K92,2e_K500`; C:`2e_K10,2e_K40,2e_K92` — × {orig,vacuum,ta}; single-inst | **supersedes** `results/unified_v2` for exactly these cells | Same-batch baseline + `2f_slru` anchor in the same run. **For `orig`, superseded by `results/unified_v4`**, which measured these arms under the same corrected tie-break alongside `layers_5`/`2d` in one batch; still canonical for `vacuum`/`ta` |
| Corrected cross-seed | `results/tiebreak_fix/seeds/seed{01..10}/summary.csv` (CI: `results/tiebreak_fix/uncertainty.csv`) | A:`2e_K500`; B:`2e_K10,2e_K500`; C:`2e_K10` — × {orig,vacuum,ta}; seeds 1–10 | **supersedes** pre-fix cross-seed `results/seeds` (non-canonical) for these cells | Each seed's corrected strategy paired with its same-seed baseline. **For `orig`, superseded by `results/unified_v4` since 2026-09-17** for the `tab:seeds` cells; still canonical for `vacuum`/`ta` |
...
| C_mixed ablation + competitive | `results/ablation_comp_v2/uncertainty.csv` (seeds `seed{01..10}/summary.csv`) | C (=C_mixed) × orig × {2d, leaf_rand_K10, leaf_freq_K10, 2e_K10, 2f_top14, 2f_top28, 2f_slru}; seeds 1–10 | **supersedes** pre-fix `results/ablation` + `results/competitive` for C/orig | Same-batch three-lever ablation + footprint-matched competitor. **Superseded by `results/unified_v4` since 2026-09-17** for `tab:ablation` and `tab:competitive`, which now take all twelve competitive cells from one batch rather than splitting A/B (`results/competitive`) from C (this batch) |
