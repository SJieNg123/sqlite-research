...
論文的核心貢獻不是「誰的 first-query 最低」,而是**把冷啟動 e2e 拆成 first-query + deliver + cold-open 三段**,揭露「first-query 贏家 ≠ e2e 贏家」。本節在原生 YCSB 上**逐張重現** Fig 13(first-query)與 Fig 14(e2e 分解),3 個代表性 workload(YC zipfian、YCu uniform、YCh01 tight hotspot)對應論文的 A/B/C 三 panel。
...
把甜蜜點策略(2d/2e)推廣到全 12 個原生 YCSB 讀取 workload × 3 layout:**2d first-query 中位 −38%(mean −33%,range [−45%, +22%])**,2e_K10 幾乎重合(中位 −38%)——證明 §2.2 的甜蜜點不是 YC 專屬。
...
論文 Fig 17 用 `leaf_rand_K10` 控制組(與 `leaf_freq_K10` **同頁數、同頁型,只差選擇準則是頻率 vs 隨機**)來隔離「頻率排名」本身的效果。本節以論文 `figures/17_lever_ablation.py` 的**精確配色與 arm 標籤**重現,擴充成 **3 workload(A·Zipfian=YC、B·uniform=YCu、C·hotspot=YCh01)× 2 layout(orig/ta)× 2 metric,每格 10-seed 均值 + 95% bootstrap CI**。
...
> **常駐 interior 骨架(2d)是穩健(12 workload 中位 −38%,95% CI 排除 0)、可擴展(6M 列上 −57%)、抗記憶體壓力(16M cap 下 100% delivery)的冷啟動預取槓桿;而整庫暖快取(2f_slru)是 first-query 陷阱(warm-e2e +4000~7000%,壓力下 delivery 崩到 13%)。好處來自 interior 結構本身——熱葉選擇與隨機對照無異。**

論文結論**不依賴其自訂 workload 產生器**;在標準 YCSB 基準下同樣穩固。
