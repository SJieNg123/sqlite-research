...
| 16 | [16_ram_pressure_sweep.py](16_ram_pressure_sweep.py) | [out/16_ram_pressure_sweep.png](out/16_ram_pressure_sweep.png) | Sub-working-set RAM-pressure sweep (cap {∞,16,12,8,6}M, below the ~17.3 MB WS). All 5 targeted strategies (layers_5/2d/2e_K10/layers_92/2e_K500, ≤2 MB hotset) keep delivery=100% and first-q flat — RAM-robustness measured across the whole targeted family, not a 3-point sample; 2f_slru (dump = whole WS) collapses once the cap drops below it. | `results/ram_pressure/cap_*/summary.csv` |
| 17 | [17_lever_ablation.py](17_lever_ablation.py) | [out/17_lever_ablation.png](out/17_lever_ablation.png) | S1 three-lever ablation (10 seeds, bootstrap 95% CI). Per workload, 4 arms isolate one selection lever each. The money pair is leaf_rand (grey ≈ baseline) vs leaf_freq (green): same page-type, same count, so the gap is the **access-frequency** signal — C's win is frequency-driven, not page-type. B/A carried by 2d (interior). Rows = orig vs ta (layout-clustering lever). | `results/ablation/uncertainty.csv` + `results/ablation_k500/uncertainty_k500.csv` |
| 18 | [18_competitive_baseline.py](18_competitive_baseline.py) | [out/18_competitive_baseline.png](out/18_competitive_baseline.png) | RR1/S4 competitive baseline (10 seeds, bootstrap 95% CI). 2f_topN = frequency-ranked partial dump (no page-type) swept over footprint; 2e_K10 (★) overlaid. e2e_warm (right) blows up with footprint (full dump +700–800% on A/B) → cost-accounting headline isn't a strawman win. ★ matches the 2f_topN curve on broad A/B (page-type not needed) but beats it on narrow C (page-type adds path-coverage robustness). | `results/competitive/uncertainty.csv` |
| 19 | [19_openwhisk_effectiveness_scatter.py](19_openwhisk_effectiveness_scatter.py) | [out/19_openwhisk_effectiveness_scatter.png](out/19_openwhisk_effectiveness_scatter.png) | OpenWhisk vs workstation effectiveness portability. Scatter of **relative** first-query reduction R (x=workstation, y=OpenWhisk, standalone) for the 55 non-lp cells both platforms ran at orig. Points hug the y=x diagonal → effectiveness ports (Spearman ρ=0.67 all / 0.74 high-conf; direction agree 42/55; median \|ΔR\|=0.114). The 3 strong-on-WS cells that flip sign on OW (C/2d, C/layers_92, C_hit/2e_K40 — all low-n / position-imbalanced) are ringed. Plots relative reductions against each other, **not** absolute µs (which are not cross-environment comparable); the 10 lp cells are compared separately by delivery order. | `deployment/openwhisk/analysis/comparison/effectiveness_ow_vs_workstation.csv` |
| 19b | [19b_openwhisk_effectiveness_slope.py](19b_openwhisk_effectiveness_slope.py) | [out/19b_openwhisk_effectiveness_slope.png](out/19b_openwhisk_effectiveness_slope.png) | Same 55-cell data as Fig 19 as a **slope chart** (more intuitive): 5 workload panels, each strategy one line from workstation R (left) to OpenWhisk R (right). Flat/high line = ports; line diving into the shaded R<0.10 zone = flip. Per-panel "N/M effective on both" count; blue=same direction, red=flips (the 3 strong exceptions dive hard in C/C_hit), faded=low-confidence. Relative reductions only, **not** absolute µs. | `deployment/openwhisk/analysis/comparison/effectiveness_ow_vs_workstation.csv` |
| 19c | [19c_openwhisk_effectiveness_bars.py](19c_openwhisk_effectiveness_bars.py) | [out/19c_openwhisk_effectiveness_bars.png](out/19c_openwhisk_effectiveness_bars.png) | Same 55-cell data as **grouped bars** (baseline = 0): 5 workload panels, per strategy a blue bar (workstation) vs orange bar (OpenWhisk) of first-query reduction R vs baseline. Bars above 0 = faster than baseline; bars dipping below 0 = slower (flip). Blue/orange track each other where effectiveness ports; orange plunges below 0 for the 3 exceptions (C/2d, C/layers_92, C_hit/2e_K40) + a couple of near-neutral C cells. Reduction-vs-baseline cancels the cross-environment offset, so blue vs orange IS comparable — **not** absolute µs. | `deployment/openwhisk/analysis/comparison/effectiveness_ow_vs_workstation.csv` |
