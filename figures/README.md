...
| 19c | [19c_openwhisk_effectiveness_bars.py](19c_openwhisk_effectiveness_bars.py) | [out/19c_openwhisk_effectiveness_bars.png](out/19c_openwhisk_effectiveness_bars.png) | OpenWhisk vs workstation effectiveness portability as **grouped bars** (baseline = 0), for the 55 non-lp cells both platforms ran at orig: 5 workload panels, per strategy a blue bar (workstation) vs orange bar (OpenWhisk) of **relative** first-query reduction R vs baseline. Bars above 0 = faster than baseline; below 0 = slower. Blue/orange track each other where effectiveness ports (Spearman ρ=0.76 all / 0.79 high-conf; direction agree 45/55; median \|ΔR\|=0.113). The 3 strong-on-WS cells that flipped sign on OW in the original low-n batches (C/2d, C/layers_92, C_hit/2e_K40) were re-run under exact position balance (campaign portability_outlier_replication) and now sit positive alongside WS; the one residual below-0 bar is the near-neutral YCh01/layers_5 (R_ws≈0.03). Reduction-vs-baseline cancels the cross-environment offset, so blue vs orange IS comparable — **not** absolute µs; the 10 lp cells are compared separately by delivery order. | `deployment/openwhisk/analysis/comparison/effectiveness_ow_vs_workstation.csv` |
