...
| 19c | [19c_openwhisk_effectiveness_bars.py](19c_openwhisk_effectiveness_bars.py) | [out/19c_openwhisk_effectiveness_bars.png](out/19c_openwhisk_effectiveness_bars.png) | OpenWhisk vs workstation effectiveness portability as **grouped bars** (baseline = 0), for the 55 non-lp cells both platforms ran at orig: 5 workload panels, per strategy a blue bar (workstation) vs orange bar (OpenWhisk) of **relative** first-query reduction R vs baseline. Bars above 0 = faster than baseline; below 0 = slower. Blue/orange track each other where effectiveness ports (Spearman ρ=0.76 all / 0.79 high-conf / 0.81 high-conf-clean; direction agree 46/55; median \|ΔR\|=0.112). Read from the **revised freeze**: the 3 strong-on-WS cells that read harmful on OW in the original low-n batches (C/2d, C/layers_92, C_hit/2e_K40) are now their exactly position-balanced replication values (positive) and 4 cells carry a `position_sensitive` marker; the one residual below-0 bar is the near-neutral YCh01/layers_5 (R_ws≈0.03 → R_ow −0.60). Reduction-vs-baseline cancels the cross-environment offset, so blue vs orange IS comparable — **not** absolute µs; the 10 lp cells are compared separately by delivery order. | `deployment/openwhisk/analysis/comparison/effectiveness_ow_vs_workstation_revised_freeze.csv` |
