#!/bin/sh
# Strategy 1c (type-aware layout_rewriter) × Workload B (uniform) and C (high-key uniform)
...
mkdir -p "$DIR/bench_records_1c_bc" "$DIR/ops_csv_1c_bc"
...
for WL_LABEL in B C; do
  ...
  for STRAT in baseline range perpage layers5; do
    ...
    for REP in 1 2 3; do
      OUT="$DIR/ops_csv_1c_bc/ops_${WL_LABEL}_${DB_LABEL}_${STRAT}_r${REP}.csv"
      LINE=$($BH --db "$DB" --workload "$WL" \
        --output "$OUT" --record-dir "$DIR/bench_records_1c_bc" \
