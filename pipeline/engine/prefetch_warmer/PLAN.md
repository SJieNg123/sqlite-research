...
| 路徑 | 角色 | 在本子專案怎麼用 |
|---|---|---|
| `pipeline/preparation/classify_pages/classify_pages.c`（binary 已 build）| 精確頁面分類（走 freelist）| A1.1 抽出 O(1) 共享 `page_classify`；精確版保留為 **oracle**（驗活表、算 precision/recall、fallback）|
| `pipeline/engine/benchmark_harness/benchmark_harness`（+ `sqlite3.c`/`sqlite3.h`）| cold-start 量測引擎 + **SQLite amalgamation** | A3 量測；**VFS shim 直接 link 這份 `sqlite3.c`** |
| `pipeline/engine/residency_checker/residency_checker` | mincore 驗 page cache 殘留 | A2 驗證 warmer 真的把頁暖進 cache |
| `pipeline/preparation/layout_rewriter/runs/evict` | `posix_fadvise(DONTNEED)` 無 root evict | A3 冷啟動前清 cache |
| `strategies/access/src/prefetch_access` | 既有 madvise prefetch 原型 | 參考；新 warmer 取代之 |
...
| SQLite | python 3.46.1；amalgamation 在 `pipeline/engine/benchmark_harness/sqlite3.{c,h}` | VFS shim 直接編這份，不另抓 |
...
```
pipeline/engine/prefetch_warmer/
  README.md                 主控執行文檔（已寫）
  PLAN.md                   本檔（A0 產出）
  src/
...
- **VFS link 方式**：用 `pipeline/engine/benchmark_harness/sqlite3.c` 靜態編進測試程式（單一 translation unit，避免版本不一致）。
- **drop-cache 無 root**：所有 A3 量測腳本沿用 `pipeline/preparation/layout_rewriter/runs/evict`（fadvise DONTNEED）+ `--benchmark-cold-advice`；量不到冷就中止，不默默量（F5）。
