...
既有的 prefetch 原型（[strategies/access/](../strategies/access/)、[strategies/slru/](../strategies/slru/)、
...
| 既有資產 | 在本子專案的角色 |
|---|---|
| [pipeline/preparation/classify_pages/classify_pages.c](../../preparation/classify_pages/) | 重構出共享 **O(1) `page_classify`** 模組（VFS 與 offline 共用）；精確版（走 freelist）保留為 **oracle**，用來驗活表正確性、算 precision/recall、活表不可用時 fallback。 |
| [pipeline/engine/benchmark_harness/](../benchmark_harness/) | 沿用做 cold-start 量測（`--cold-advice`、`--drop-caches-script`、ops/latency/majflt）。 |
| [pipeline/engine/residency_checker/](../residency_checker/) | 驗證 warmer 真的把目標頁暖進 page cache（mincore）。 |
| `pipeline/preparation/layout_rewriter/runs/evict`（posix_fadvise DONTNEED）| 無 sudo 冷啟動的 evict 工具。 |
