...
把外部 prior-art 的**選頁/遞送核心**移植到同一 harness（SQLite + OS page cache + cold-start critical path），做同 batch 配對比較。**不是跑對方系統本尊，只重現核心 + 剝除其編排。** 資料源 `results/baselines_v2/summary.csv`（`tools/baselines_v2.sh`，A/B/C × orig，pread 10 / async 10）。

> ⚠️ **機器狀態錨點：** 本批的機器狀態比 `results/main` **快 ~1.3×**（2f_slru async 錨點：batch A/B/C = **96/99/97 µs** vs results/main **126/126/124 µs**，ratio 1.28–1.31）。故本節**絕對 µs 只能批內配對比較**，跨到 results/main 需乘 ~1.3。baseline fq（batch）= A 507 / B 745 / C 1070。

#### libprefetch-style delivery arm — `lp_sorted` / `lp_shuf`

重現 libprefetch（VanDeBogart+09）的核心：**application-provided access list ＋ 按磁碟 offset 排序 ＋ 批次同步載入（sync pread）**。**選頁與遞送嚴格分離**：兩個 arm 的 hotset 內容**完全等同 `2f_slru`**（checksum 相同），差別**只在 warmer 的 pread 遞送順序**——`lp_sorted`=offset 升序（C-LOOK 精神）、`lp_shuf`=seeded shuffle（摧毀 offset locality 的對照）。實作走 file-side `build_hotset(order=)`，**warmer.c 一行不改**。剝除的編排：kernel reorder buffer / infill / AIMD contention controller（HDD 時代機制）。commit `5ac88da`/`850b27b`。

**主度量：Δdeliver = deliver_us(shuf) − deliver_us(sorted)**（`fq` 為 control，兩 arm 應相等——因載完皆 100% resident）。**結果（pread，orig，`results/baselines_v2`）：**

| Workload | deliver sorted | deliver shuf | **ratio** | fq sorted / shuf |
|---|---:|---:|---:|---:|
| A | 17,974 µs | 273,596 µs | **15.2×** | 101 / 108 |
| B | 18,556 µs | 268,152 µs | **14.5×** | 99 / 111 |
| C | 2,817 µs | 29,301 µs | **10.4×** | 97 / 101 |

**結論：**
- **NVMe 上 offset 排序遞送快 10–15×**，效應**全落在 `deliver_us`、fq 不變**（載完 cache 狀態相同）——libprefetch 的 seek 收益在 NVMe 上**並非消失**，而是被 kernel sequential-readahead + 隨機 4KB pread 懲罰放大。
- 診斷（`/usr/bin/time -v` File system inputs）：sorted 與 shuf 讀取**相同裝置位元組（~18 MB ≈ 工作集）** → sorted 是「串流工作集」、非「掃更大 span」；15× 純為順序（readahead 合併成少數大 I/O）vs 隨機 IOPS。**readahead 即隱式 coalesce**，故顯式 coalesce 未實作（僅列 Phase 1b 分解用）。
- async(fadvise) arm 無此效應（僅發 hint）→ 懲罰專屬**同步 pread** 路徑，即 libprefetch 模型。

#### learned-style arm — `learned_markov`（Chen-inspired transition baseline）+ `frequency`

**Chen 等（ICDE 2021）的 formulation 啟發**的輕量 baseline（**非重現**）：保留「從歷史 trace 學 page 轉移、預測下一批頁 + held-out 評估」，把不可得的 neural model 換成透明的**一階 Markov transition model**；未重現 Decision Module / 背景執行緒 / neural 架構；只用 page-access context。實作 `strategies/learned/{gen_pageseq,train_markov}.py`：每 query 為獨立 episode `START→root→interior→leaf→END`（僅 op 內建 transition）、`P(q|p)=count/Σ`、**從 START 做 finite-horizon expected-visit 展開**（horizon=max深度+1，**非** stationary）；hotset 取 `_scores.csv` top-N。**held-out：LOSO**（測 master=test seed 1、訓練 seeds 2..10），硬 assert `test∉train`。`frequency_N` 是**獨立 code path**的分析臂（hotset 取 `_marginal.csv` top-N）。footprint 與 `2f_topN` 對齊（同頁數同 bytes）。commit `a98e673`（8 道 validation gate 全過）。

**結果（async，deployment-relevant，orig，`results/baselines_v2`）：**

| Workload | learned_markov_14 fq / e2e | 2f_top14 fq / e2e | 2e_K10 fq / e2e |
|---|---:|---:|---:|
| A | 356 / 435 | 380 / 459 | 376 / 475 |
| B | 409 / 490 | 409 / 489 | 408 / 511 |
| C | 181 / 262 | 183 / 263 | 183 / 264 |

**Jaccard（hotset 相似度，離線分析，非性能指標）：** `J(learned_markov, frequency) = 1.000`（全 workload/N）；`J(learned_markov, 2f_topN)`：A/B N14 **0.47 / 0.56**、N28 0.22；**C N14/N28 = 1.00**。

**結論（僅陳述實測，不外推）：**
- **learned_markov 在 A/B/C 三 workload 的 async fq/e2e 都 ≈ 2f_topN（多在噪音內；A 略優，因 LOSO 9-seed 池化的頻率估計較單 seed 穩）**——即此 transition baseline 在冷啟動 regime 的可用輸出，落在 `2f_topN` 已覆蓋的頻率排名範圍。
- **`learned_markov` 與 `frequency` 選到同一組頁（J=1.0）**：此為當前**3 層固定深度 tree 的觀測性質**（每頁只在單一深度 → finite-horizon expected-visit score = 正規化 visit frequency），由**兩條獨立 code path** 算出——**非**「必等於 frequency」的普遍宣稱，也不外推到其他模型。
- **C 上 `J(learned_markov, 2f_topN)=1.0`**：C 的 top-14 被穩定的 interior 結構主導（跨 seed 不變）→ pooled 與 master 選出同集合；A/B 較低是 hot-leaf 隨 seed 抽樣漂移（interior 部分仍吻合）。
- **Workload E 未支援**：range scan 非 3-page episode，`gen_pageseq` 對 `scan` fail-loud；需真正 range-query 頁序列重建才支援（**不簡化**）。
- 完整 LOSO（跨所有 test seed）、held-out coverage / precision 等離線指標尚未跑；訓練為 offline preprocessing（runtime 用預生成 static hotset、無 online inference）。
...
| Baseline-v2 | lp_sorted / lp_shuf（libprefetch）| **NVMe 上 offset 排序遞送 Δdeliver 10–15×**，效應全在 deliver、fq 不變（`results/baselines_v2`）|
| Baseline-v2 | learned_markov（Chen-inspired）| async fq/e2e **≈ 2f_topN**（A/B/C 多在噪音內）；hotset≡frequency(J=1.0，tree 性質)；E 未支援 |
| Baseline-v2 | frequency（分析臂）| marginal top-N，獨立 code path；與 learned_markov 同集合（J=1.0）|
