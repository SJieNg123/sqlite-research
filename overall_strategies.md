...
**結論：** N=5 是 syscall overhead 與 coverage 的最佳折衷。**目前所有實驗的
canonical prefetch 策略**。配合 type-aware layout 可達 -69%。
...
**狀態：** 已完成，在 Workload A (Zipfian) 和 Workload B (Uniform) 上跑過。
**結果：**
- **First-query latency 大勝**：A -94%（255 → 15 µs）、B -94%（251 → 14 µs）
- **但 prefetch 自己 4,000+ syscalls 花 ~7.5 ms** → 端到端 cold start 比
  baseline 慢 30×
- **全 workload 跑完反而省 38%**（255 ms vs 413 ms），因為所有 touched leaf
  都被 pre-warm
...
| Layout | 1b SQLite VACUUM | ✅ 已驗證（A/B baseline +5~8%、C baseline -6%；對 layers_5 效益影響小）|
...
| Prefetch | 2c Layers N（structure） | ✅ 已完成 + 找到 N=5 甜蜜點 |
| Prefetch | 2d Access pattern，interior only | 🚧 未實作 |
| Prefetch | 2e Access pattern，interior + leaf | 🚧 未實作 |
| Prefetch | 2f SLRU（mincore-dumped resident set）| ✅ 已完成 |
