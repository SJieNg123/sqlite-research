...
三個 workload 上跑過。
**結果：**
- **First-query latency 在三個 workload 上一致大勝 -94%**：A 251 → 14 µs、
  B 255 → 15 µs、C 250 → 16 µs
...
| Prefetch | 2c Layers N（structure） | ✅ 已完成 N sweep on A/B/C（N=5 只是 A 的甜蜜點；B 全 plateau、C 要 N=92）|
