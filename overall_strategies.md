...
> **統一 hotset**：`hotpages_*.csv` 由 `run_experiment.py --regen-hotsets`（全機 `drop-caches` warmup）重產並 checksum 凍結（`results/main/hotset_freeze.sha256`）。下表 async 臂、vs baseline、`cold_pct`=0。
...
> **統一 hotset**：2e = resident interior ∪ top-K leaves（leaf 由 workload 頻率算,deterministic）；同一 `--regen-hotsets` 重產並凍結。下表 async 臂、first-q %（vs baseline）。
...
> **統一 hotset**：2f = 整個 resident working set（base 殘留 `hotpages_*.csv`）；`--regen-hotsets` 重產並凍結。交付走統一 `warmer`，`preproc_us` 取 live `warmer_us`。下表 async 臂、vs baseline。
